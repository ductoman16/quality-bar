...
function validApplicabilityEvidence(value: any) {
  if (!record(value)) {
    return false;
  }
...
function validApplicability(value: any) {
  const common =
    record(value) &&
...
function validCriterionResult(value: any) {
...
function validLocation(value: any) {
  if (
    !record(value) ||
...
export const validProcess = (value: any) =>
  record(value) &&
  ((value.kind === "exit" &&
...
const validMeasurements = (value: any) =>
  record(value) &&
  exact(value, [
    "codex_cli_version",
    "duration_ms",
...
function validReviewRun(value: any, evaluationId: string) {
...
const validFileChange = (value: any) =>
  record(value) &&
  exact(value, [
    "added",
    "after_path",
...
export function validEvaluationResult(value: any, evaluationId: string) {
  if (
    !record(value) ||
    !exact(value, [
      ...
      (run) =>
        same(
          ...
          value.criterion_results.filter(
            (criterion: any) => criterion.review_run_id === run.id,
          ),
        ) &&
        same(
          run.findings,
          value.findings.filter(
            (finding: any) => finding.review_run_id === run.id,
          ),
        ),
    ) &&
