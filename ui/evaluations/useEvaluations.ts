...
import { validAnalytics } from "../analytics/contract.ts";
...
const timestamp = (evaluation: any) =>
...
const epoch = (value: any) => {
  const parsed = value ? new Date(value).getTime() : NaN;
  return Number.isSafeInteger(parsed) && parsed >= 0 ? String(parsed) : "";
};
export function useEvaluations(csrfCookieName: any) {
  ...
  async function refresh({
    ...
    if (poll) {
      const incoming = new Map(
        collection.items.map((evaluation: any) => [evaluation.id, evaluation]),
      );
      const currentIds = evaluations.value
        .slice(0, collection.items.length)
        .map(({ id }) => id);
      const incomingIds = collection.items.map(({ id }: any) => id);
    ...
    evaluations.value =
      cursor && !replace
        ? [
            ...
            ...items.filter(
              (item: any) =>
                !evaluations.value.some(({ id }) => id === item.id),
            ),
          ]
        : items;
  ...
  async function mutate(evaluation: any, action: any) {
    let failureMessage = "";
    try {
