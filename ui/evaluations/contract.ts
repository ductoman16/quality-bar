...
export { validReviewRunDiagnostics } from "./diagnostics-contract.ts";
export { validEvaluationResult } from "./result-contract.ts";
...
const positive = (value: any) => Number.isSafeInteger(value) && value > 0;
const publicationError = (value: any) => value === null || validError(value);
const delivery = (value: any) =>
  record(value) &&
...
const publication = (value: any, finding = false) =>
  delivery(value) &&
  exact(value, [
    "attempt_count",
    "connection_identity",
    ...
    : value.publication_status === "waiting"
      ...
      : value.publication_status === "succeeded"
        ...
        : value.publication_status === "unavailable" &&
          ...
          value.published_at === null &&
          value.error !== null);
const validFeedback = (value: any) =>
  record(value) &&
  exact(value, ["aggregate", "findings"]) &&
  publication(value.aggregate) &&
  Array.isArray(value.findings) &&
  value.findings.every((item: any) => publication(item, true));
const validCommitStatus = (value: any, headCommit: string) =>
  delivery(value) &&
  exact(value, [
    "attempt_count",
    "connection_identity",
  ...
  (value.publication_status === "waiting"
    ...
    : value.publication_status === "succeeded"
      ...
      : value.published_at === null && value.error !== null);
function validCounts(value: unknown, names: string[]) {
  return (
    record(value) &&
...
export function validMonitor(value: unknown) {
  return (
    record(value) &&
...
export function validEvaluation(value: unknown) {
...
export const validEvaluationMutation = (
  value: any,
  evaluationId: string,
  action: string,
) =>
...
export function validCollection(value: unknown) {
  return (
    record(value) &&
...
export function nodeVisualState(node: unknown) {
  if (!record(node)) {
    return "pending";
  }
