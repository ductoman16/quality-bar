...
import { validPollingDelivery } from "./polling-contract.ts";
const CATALOG = {
...
export const validModelCatalog = (value: any) =>
  record(value) &&
...
const status = (value: any, allowed: any) =>
  record(value) && allowed.includes(value.status);
const storageError = (value: any) =>
  value === null ||
  (record(value) &&
    exact(value, ["code", "detail"]) &&
    nonempty(value.code) &&
    nonempty(value.detail));
const application = (value: any) =>
  record(value) &&
  exact(value, [
    "application_version",
    "error",
...
const storage = (value: any) =>
  ...
  new Set(value.filesystems.map(({ filesystem }: any) => filesystem)).size ===
    2 &&
...
export const validSystem = (value: any) =>
  record(value) &&
...
export const validConfiguration = (value: any, models: any) =>
  record(value) &&
...
export const validCodexConfiguration = (value: any) =>
  validConfiguration(
    { configuration: value, configured: true },
    CATALOG.models,
  );

export const validConfigurationChange = (value: any, models: any) =>
  record(value) &&
