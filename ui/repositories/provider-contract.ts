import { count, exact, httpsUrl, nonempty, record, uri } from "../contract.ts";

export type Validator = (value: any) => boolean;
const positive: Validator = (value) => Number.isSafeInteger(value) && value > 0;
const optionalCount: Validator = (value) => value === null || count(value);
const codedError: Validator = (value) =>
  record(value) &&
  exact(value, ["code", "message"]) &&
  nonempty(value.code) &&
  nonempty(value.message);
const health: Validator = (value) =>
  value.health === "healthy"
    ? value.health_error === null
    : value.health === "error" && codedError(value.health_error);
const permissions: Validator = (value) =>
  record(value) &&
  exact(value, ["admin", "pull", "push"]) &&
  value.admin === true &&
  value.pull === true &&
  value.push === true;
const principal: (value: any, github?: boolean) => boolean = (
  value,
  github = false,
) =>
  record(value) &&
...
const connectionPrincipal: Validator = (value) =>
  record(value) && positive(value.id) && nonempty(value.login);

const pollingError: Validator = (value) => value === null || codedError(value);
const providerPolling: (value: any, github: boolean) => boolean = (
  value,
  github,
) =>
  record(value) &&
...
const polling: Validator = (value) => providerPolling(value, false);
const githubPolling: Validator = (value) => providerPolling(value, true);
const pollingFailure: Validator = (value) =>
  value === null ||
  (record(value) &&
    exact(value, [
      "error",
...
const githubCapabilities: Validator = (value) =>
  record(value) &&
  exact(value, [
    "aggregate_feedback",
    "branch_access",
...
const githubPermissions: Validator = (value) =>
  record(value) &&
  exact(value, [
    "contents",
    "issues",
...
const githubRepository: Validator = (value) =>
  record(value) &&
  exact(value, [
    "api_url",
    "clone_url",
...
const githubVerification: Validator = (value) =>
  ...
  new Set(value.repositories.map((repository: any) => repository.id)).size ===
    value.repositories.length &&
  value.repositories.every((repository: any) =>
  ...
  new Set(value.repository_checks.map((check: any) => check.repository_id))
    .size === value.repository_checks.length &&
  value.repository_checks.every((check: any) =>
  ...
  (value.outcome === "success"
    ? value.api_profile === "github-rest:2026-03-10" &&
      ...
      value.repository_checks.every((check: any) => check.outcome === "success")
    : value.outcome === "error" &&
...
export const validGitHubConnection = (value: any) =>
  value === null ||
  (record(value) &&
    exact(value, [
      "api_profile",
...
const forgejoRepository: Validator = (value) => {
  if (!record(value)) {
    return false;
  }
...
const forgejoVerification: Validator = (value) =>
  record(value) &&
  exact(value, [
    "api_profile",
    "capabilities",
  ...
  (value.outcome !== "success" ||
    value.repositories.every(
      (repository: any) => repository.outcome === "success",
    )) &&
  (value.outcome === "success"
...
export const validForgejoConnection = (value: any) =>
  value === null ||
  (record(value) &&
    exact(value, [
      "api_profile",
...
export const validForgejoChoices = (value: any) =>
  Array.isArray(value) &&
  value.length > 0 &&
  value.every(
    (choice: any) =>
...
export const validManifestContinuation = (value: any) =>
  record(value) &&
...
export const validLifecycleChange = (
  provider: string,
  method: string,
  value: any,
) =>
...
export const forgejoConnectionUsed = (value: any) =>
  value.verification_history.some((item: any) => item.repositories.length);
