...
it("validates discovery and submits a Forgejo Connection mutation", async () => {
  ...
  const registration: any = wrapper
    .findAll("form")
...
it("starts the GitHub App manifest flow with its canonical response", async () => {
  ...
  const wrapper: any = mount(ProviderConnections, {
    props: { csrfCookieName: "qb_csrf" },
  });
  await flushPromises();
  await wrapper
    .findAll("form")
    .find((form: any) => form.text().includes("Connect GitHub App"))
    .trigger("submit");
  await flushPromises();
  expect(
...
it("combines Forgejo reactivation and authority failures", async () => {
  ...
  await wrapper
    .findAll("form")
    .find((form: any) => form.text().includes("Reactivate Forgejo"))
    .trigger("submit");
  await flushPromises();
...
it("rejects a null successful Forgejo registration", async () => {
  ...
  await wrapper
    .findAll("form")
    .find((form: any) => form.text().includes("Register selected Forgejo"))
    .trigger("submit");
  await flushPromises();
  expect(wrapper.emitted("error").at(-1)).toEqual([
...
it("refreshes a committed Forgejo retirement without hiding a lost response", async () => {
  ...
  await wrapper
    .findAll("button")
    .find((button: any) => button.text() === "Retire Forgejo Connection")
    .trigger("click");
