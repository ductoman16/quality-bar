...
const formWith = (wrapper: any, text: any) => {
  const form = wrapper
    .findAll("form")
    .find((item: any) => item.text().includes(text));
  if (!form) {
...
it("covers version, assignment, archival, and deletion controls", async () => {
  ...
  const wrapper: any = mount(ReviewDetailView, {
  ...
  await wrapper
    .findAll("button")
    .find((button: any) => button.text() === "Archive")
    .trigger("click");
  await flushPromises();
  ...
  const deleteButton: any = wrapper
    .findAll("button")
    .find((button: any) => button.text() === "Delete Review");
...
it("surfaces malformed mutation success and reconciles ambiguous deletion", async () => {
  ...
  expect(
    vi
      .mocked(fetch as any)
      .mock.calls.filter(([path]: any) => path === "/api/v1/reviews"),
  ).toHaveLength(2);
  wrapper.unmount();
});
