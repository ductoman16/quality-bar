...
4. **Act on Findings.** Fix blocking Findings. Fix advisory Findings or propose a Waiver Request; only an accepted Waiver Decision clears an unfixed advisory Finding. A changed head returns to step 2. Done when no blocking or unwaived advisory Findings remain.
