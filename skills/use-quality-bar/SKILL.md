...
2. **Prepare the change.** Implement and test against the Repository Guidance; the Evaluation will review this exact change against those Reviews and Criteria. Before submitting the Evaluation, re-read the guidance and fix avoidable gaps, then push and resolve the exact base and head commits. Done when both commits exist in the Forge and every active Criterion has been checked against the change.
...
4. **Act on Findings.** Fix blocking Findings. A Waiver Request for an advisory Finding is appropriate only in exceptional circumstances: the reviewer's judgment is incorrect for this exact change, or implementing the Finding would harm its correctness or maintainability. Effort alone does not justify a waiver; only an accepted Waiver Decision clears an unfixed advisory Finding. A changed head returns to step 2. Done when no blocking or unwaived advisory Findings remain.
5. **Request a waiver.** Only advisory Findings are eligible. When step 4 identifies exceptional circumstances, submit a Waiver Request with the rationale, poll Quality Bar's Waiver Adjudication until terminal, and read every Waiver Decision. Done when every request has a terminal Decision.
