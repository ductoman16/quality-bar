...
4. **Act on Findings.** Fix blocking Findings. Fix advisory Findings or, only in exceptional circumstances, propose a Waiver Request: the reviewer's judgment is incorrect for this exact change, or implementing the Finding would harm its correctness or maintainability. Effort alone does not justify a waiver; only an accepted Waiver Decision clears an unfixed advisory Finding. A changed head returns to step 2. Done when no blocking or unwaived advisory Findings remain.
