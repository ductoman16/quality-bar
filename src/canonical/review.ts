import { closedObject, withValidationError } from "./schema.ts";
import { EVALUATION_CANCELLATION_CODES } from "../evaluation/evaluation-cancellation-reason.ts";
...
export function canonicalReviewSchemas() {
  ...
  const criteria = (items: Record<string, unknown>) =>
    withValidationError(
...
export function canonicalWaiverSchemas() {
  ...
  const operational = (properties: Record<string, any>) =>
    closedObject(
