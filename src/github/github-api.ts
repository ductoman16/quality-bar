...
import { createGitHubApiRequest } from "./github-api-request.ts";
import { GitHubConnectionError } from "./github-connection-error.ts";
import * as gitVerification from "./github-git-verification.ts";
import { verifyGitHubRepositories } from "./github-repository-verification.ts";
import { createGitHubPullRequestReader } from "./github-pull-request-api.ts";
import { createGitHubInstallationToken } from "./github-installation-token.ts";
import { createGitHubCommitStatusPublisher } from "./github-commit-status-api.ts";
import { createGitHubFeedbackPublisher } from "./github-feedback-api.ts";
import { createGitHubManifestExchange } from "./github-manifest-exchange.ts";
import { verifyRepositoryRead } from "../repository/repository-git.ts";
...
function onlyGitHubMandatedEvents(value: unknown) {
  return (
    Array.isArray(value) &&
...
function privateOrUnspecified(value: unknown) {
...
function appJwt(appId: number, pem: string, now: number) {
...
export function createGitHubVerifier({
  ...
  fetch?: typeof fetch;
  now?: () => number;
  verifyGit?: typeof verifyRepositoryRead;
} = {}) {
  if (
  ...
  const verifier = {
    ...
    async verifyInstallation(
      ...
      const installation = object(
        installations.find(
          (candidate: unknown) => object(candidate)?.id === installationId,
        ),
      );
      if (
        !installation ||
