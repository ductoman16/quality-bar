...
import { createGitHubVerifier } from "./github-api.ts";
import { createGitHubCallbackFailureStore } from "./github-callback-failure.ts";
// prettier-ignore
import { GITHUB_API_PROFILE, GITHUB_REQUIRED_PERMISSIONS } from "./github-app-manifest.ts";
import {
  createGitHubConnectionCredentialCipher,
  validatePersistedGitHubCredentials,
} from "./github-connection-credential.ts";
import {
  GitHubConnectionError,
  failGitHubConnection as fail,
} from "./github-connection-error.ts";
import {
  createGitHubConnectionFlowToken,
  createGitHubConnectionTimestamp,
  takeGitHubConnectionFlow,
} from "./github-connection-flow.ts";
import { readGitHubConnection } from "./github-connection-read.ts";
import { reactivateGitHubConnection } from "./github-connection-reactivation.ts";
import { startGitHubConnection } from "./github-connection-start.ts";
import {
  removeNeverUsedGitHubConnection,
  retireGitHubConnection,
} from "./github-connection-lifecycle.ts";
import { createGitHubRepositorySelector } from "./github-repository-registration.ts";
import { createGitHubRepositoryGitCredential } from "./github-repository-git-credential.ts";
import { createGitHubPollingRunner } from "./github-polling-runner.ts";
import { createGitHubConnectionRotation } from "./github-connection-rotation.ts";
import { stopFor as stop } from "./github-connection-shutdown.ts";
import { createGitHubPublicationServices } from "./github-publication-services.ts";
import { resumeGitHubDeliveries } from "./github-delivery-recovery.ts";
import { assertGitHubVerifier } from "./github-connection-dependencies.ts";
export { GitHubConnectionError } from "./github-connection-error.ts";
...
export type GitHubVerifier = {
  ...
  exchangeManifest: (code: string) => Promise<any>;
  listPullRequests: (
    credential: any,
    installationId: number,
    repository: any,
  ...
  verifyInstallation: (credential: any, installationId: number) => Promise<any>;
  verifyRepositories: (
    credential: any,
    installationId: number,
...
export function createGitHubConnectionService(
  ...
  // prettier-ignore
  const cipher = createGitHubConnectionCredentialCipher(masterKey, { onSecret: registerSecret });
  ...
  return {
    ...
    recordCallbackFailure: (error: Error & { code: string }) =>
      callbackFailures.record(error),
    consumeCallbackFailure: (receipt: string) =>
      callbackFailures.consume(receipt),
    async completeManifest({ code, state }: { code: string; state: string }) {
      if (
        typeof code !== "string" ||
        code.length === 0 ||
        typeof state !== "string"
    ...
    async completeInstallation({
      ...
      // prettier-ignore
      [flow.credential.client_id, flow.credential.pem].forEach((secret) => registerSecret?.(typeof secret === "string" ? secret : ""));
    ...
    async reactivate(request: unknown) {
    ...
    retire: (request: unknown) => retireGitHubConnection(durableCore, request),
