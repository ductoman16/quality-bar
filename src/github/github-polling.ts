...
} from "./github-polling-generation.ts";
import {
  githubPollingFailure,
  nextGitHubAttemptAt,
} from "./github-polling-failure.ts";
import { pullRequestSnapshot } from "./github-pull-request-snapshot.ts";
export { pullRequestSnapshot } from "./github-pull-request-snapshot.ts";
export {
...
export function createGitHubPollingService(
  ...
  async function prepare(
    input: { connection: any; credential: any; repositories: any[] },
    {
      ...
      onFailure?: (
        failure: GitHubConnectionError,
        commit: (transaction: any) => boolean,
      ) => void;
    } = {},
  ) {
    if (
    ...
    const registeredIds = new Set(
      durableCore
        ...
        .map(
          ({ forge_repository_id: id }: { forge_repository_id: number }) => id,
        ),
    );
    if (!ignoreGate) {
    ...
    const snapshots: { forgeRepositoryId: number; snapshot: unknown[] }[] = [];
    try {
      for (const repository of input.repositories) {
        snapshots.push({
      ...
      const commitFailure = (transaction: any) => {
        if (
          !claimGitHubPollingGeneration(
            transaction,
            input.connection.id,
