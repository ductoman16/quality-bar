...
function authenticatedData(connection: { appId: number; id: string }) {
  return Buffer.from(
    JSON.stringify([
...
function validateCredential(credential: GitHubCredentialPayload) {
  if (
    !credential ||
...
export function createGitHubConnectionCredentialCipher(
  ...
  const rememberCredential = (credential: GitHubCredentialPayload) => {
...
export function validatePersistedGitHubCredentials(
  durableCore: any,
  cipher: {
    decrypt: (
  ...
  try {
    for (const row of durableCore.all(
     ...
     JOIN github_connection_credentials
      ...
      cipher.decrypt(
        { appId: row.app_id as number, id: row.id },
