...
} from "../forge/connection/lifecycle-guards.js";
...
function loadConnection(durableCore) {
  return /** @type {any} */ (
...
const isRetired = (connection) => connection.lifecycle === "retired";

/** @param {any} durableCore @param {any} connection */
function hasActiveDependents(durableCore, connection) {
  return (
    durableCore.all(
...
/** @param {any} transaction @param {any} connection */
function commitRetirement(transaction, connection) {
  transaction.run(
...
export function removeNeverUsedGitHubConnection(durableCore) {
  runConnectionRemoval({
    ...
    loadConnection: (core) =>
      /** @type {any} */ (
        core.all("SELECT id FROM github_connections LIMIT 1")[0]
      ),
    notFoundError: {
      code: "github_connection_not_found",
      message: "GitHub Connection is not configured",
