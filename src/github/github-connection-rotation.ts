...
import {
  ...
  unionPositiveIds,
  uniquePositiveIds,
} from "./github-connection-rotation-support.ts";
import {
  GITHUB_API_PROFILE,
...
export async function rotateGitHubConnection(
  ...
  const knownRepositoryIds = allRepositories.map(
    (repository: Record<string, any> | undefined) =>
      Number(repository?.forge_repository_id),
  ) as number[];
  ...
  let preparedBaseline: any | undefined;
  try {
    const repositoryIds =
    ...
    const evidence = repositories as any[];
...
export function createGitHubConnectionRotation(
  dependencies: Parameters<typeof rotateGitHubConnection>[0],
) {
  return (request: unknown) => rotateGitHubConnection(dependencies, request);
}
