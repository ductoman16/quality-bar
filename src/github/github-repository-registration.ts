...
import { recordGitHubConnectionVerification } from "./github-connection-verification.ts";
import { normalizeGitHubRepositorySelection } from "./github-repository-selection.ts";
import {
  createGitHubRepositorySelectionCommit,
  requireCurrentGitHubRepositoryConnection,
  requireCurrentGitHubRepositories,
} from "./github-repository-selection-commit.ts";
import { readGitHubRepositorySelectionSnapshot } from "./github-repository-selection-snapshot.ts";
import {
  validGitHubRepositoryEvidence,
  verifiedGitHubRepositoryEvidence,
} from "./github-verification-error.ts";
import { githubVerificationErrorScope } from "./github-verification-scope.ts";
import {
  readRepositoryResource,
  REPOSITORY_SELECTION,
} from "../repository/repository-resource.ts";
...
export function createGitHubRepositorySelector(
  ...
  },
  {
    ...
    verifier: {
      ...
      verifyRepositories?: (...parameters: any[]) => Promise<any>;
    };
  },
) {
  ...
  return async function selectRepositories(
    request: unknown,
    trigger: "enablement" | "repository_selection" = "repository_selection",
    { deferCommit = false }: { deferCommit?: boolean } = {},
  ) {
    ...
    try {
      ...
      let failureCommit: ((transaction: any) => void) | undefined;
        ...
        if (scope) {
          ...
          const failure = recordGitHubConnectionVerification(
            durableCore,
            {
              ...
              timestamp,
              trigger: trigger as "enablement" | "repository_selection",
            },
            { defer: true },
          );
    ...
    if (
      ...
      repositories.some(
        (repository) =>
          !validGitHubRepositoryEvidence(
            repository,
            connection.principal_login as string,
          ),
      ) ||
    ...
    const removedVerifications: ReturnType<
      typeof recordGitHubConnectionVerification
    >[] = [];
