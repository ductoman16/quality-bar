...
import { DEFINITIVE_GITHUB_DELIVERY_ERRORS } from "./github-delivery-errors.ts";

const MAXIMUM_DELAY_MS = 60 * 60 * 1_000;
const DEFINITIVE_FAILURES = new Set(DEFINITIVE_GITHUB_DELIVERY_ERRORS);

export function githubDeliveryFailure(
  error: unknown,
  ...
  const codedError =
    ...
    DEFINITIVE_FAILURES.has((error as any).code)
  ...
  const providerGate =
    ...
    (Number.isSafeInteger(failure.nextAttemptAt) ||
      [403, 429].includes(failure.responseStatus as number));
  return {
    code: failure.code,
...
export function ensureGitHubDelivery(
  durableCore: any,
...
export function failGitHubDelivery(
  ...
  const nextAttemptAt = failure.definitive
    ...
    : nextGitHubDeliveryAttemptAt(
        attemptedAt,
        (delivery.attempt_count as number) + 1,
        failure,
      );
