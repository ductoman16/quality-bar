...
import { attemptGitHubDelivery } from "./github-delivery-service.ts";
import { recordGitHubDeliveryHealth } from "./github-delivery-service.ts";
import {
  readAggregateDeliveryTarget,
  readInlineDeliveryTarget,
} from "./github-feedback-delivery-target.ts";
import { createForgeFeedbackMaterializer } from "../forge/feedback/materializer.ts";
...
function materializeGitHubFindingFeedback(
  durableCore: any,
  bundle: any,
  findings: any[],
) {
  const fileChanges = new Map<string, any>(
    durableCore
      ...
      .map((fileChange: any) => [fileChange?.id, fileChange]),
  );
...
function validateGitHubFeedbackVerifier(verifier: any) {
  if (
...
function createGitHubAuthentication(cipher: any, bundle: any) {
  return () => {
    const credential = cipher.decrypt(
      {
        appId: bundle.app_id as number,
        id: bundle.connection_id as string,
      },
      bundle.encrypted_credential as string,
    );
    return {
...
function attemptGitHubAggregateDelivery({
  ...
  const deliverAggregate = (method: any, deliveryTarget: string) => {
  ...
  return attemptGitHubDelivery(durableCore, {
    connectionId: bundle.connection_id as string,
    create: (target) =>
    ...
    onDefinitive: (transaction, failure, attemptedAt) => {
      ...
      recordGitHubDeliveryHealth(
        transaction,
        bundle.connection_id as string,
        attemptedAt,
        failure,
      );
    },
...
function attemptGitHubFindingDelivery({
  ...
  const deliverInline = (method: any, deliveryTarget: string) => {
  ...
  return attemptGitHubDelivery(durableCore, {
    ...
    sourceId: row.finding_id as string,
