...
import { EvaluationError } from "../evaluation/evaluation-validation.ts";
import { fileChangesFromGitNameStatus } from "../file-change.ts";
import { ReviewRunExecutionError } from "./review-run-result.ts";

function lineCount(contents: Buffer): number | null {
...
export function readReviewRunFileChanges(
  checkoutPath: string,
  baseCommit: string,
  headCommit: string,
) {
