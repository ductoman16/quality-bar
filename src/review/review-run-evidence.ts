...
export function readTerminalTokenCounters(stdout: string) {
...
export function captureEvidenceCompletionFailure(
  evidenceService: { complete: (claim: unknown, facts: unknown) => void },
  claim: unknown,
  process: { code?: number | null; signal?: string | null } | undefined,
  stdout: string,
) {
  try {
...
function assertClaim(claim: unknown) {
...
export function createReviewRunEvidenceService(durableCore: any) {
  function assertAuthoritativeStartedRun(transaction: any, claim: any) {
  ...
  return {
    ...
    complete(claim: unknown, facts: any) {
      assertClaim(claim);
