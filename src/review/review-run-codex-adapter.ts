...
import {
  ...
  prepareCodexProcess,
} from "../codex/codex-process-supervisor.ts";
import {
...
} from "./review-run-codex-command.ts";
import {
...
} from "./review-run-codex-process.ts";
import * as group from "../codex/codex-execution-process-group-tracking.ts";
import * as deadline from "./review-run-deadline.ts";
import * as evidence from "./review-run-evidence.ts";
import { createReviewRunProcessGroupTermination } from "./review-run-process-group.ts";
import { ReviewRunExecutionError } from "./review-run-result.ts";
import { settleSubmissionTerminal } from "./review-run-codex-submission-terminal.ts";
import { openReviewRunSubmissionChannel } from "./review-run-submission-channel.ts";
...
export async function runReviewRunCodex({
...
}: import("./review-run-codex-options.ts").ReviewRunCodexOptions) {
  ...
  const diagnosticFailures: Error[] = [];
  ...
  try {
    ...
    let signalDeadline: (value?: void) => void = () => {};
    ...
    let terminal:
      | { kind: "cancellation" }
      | { kind: "deadline" }
      | { kind: "process"; result: any }
      | { kind: "process-error" }
      | { kind: "submission"; result: "accepted" | "failed" }
      | { kind: "transcript" };
    try {
    ...
    let acceptedTerminationFailure: Error | undefined;
    let submissionTerminationFailure: Error | undefined;
    let processErrorTerminationFailure: Error | undefined;
    if (
      (accepted ||
        failedSubmission ||
        processError ||
    ...
    if ((terminal.kind === "process" || processError) && !accepted) {
      const exit = terminalProcess as any;
