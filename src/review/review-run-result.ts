...
export function validateReviewRunSubmission(
  candidate: unknown,
  criteria: { criterion_id: string; impact: string }[],
  fileChanges: {
    id: string;
  ...
  if (
    ...
    !Array.isArray(
      (candidate as { criterion_results?: unknown }).criterion_results,
    )
  ) {
    fail(
  ...
  const results = (candidate as any).criterion_results as any[];
  ...
  const submittedIds = results.map(
    ({ criterion_id: criterionId }) => criterionId as string,
  );
  if (
...
function validateFinding(finding: any, fileChanges: any[]) {
  if (
    !finding ||
...
export { createReviewRunResultService } from "./review-run-result-service.ts";
