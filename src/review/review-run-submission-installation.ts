...
const submitPath = fileURLToPath(
  new URL("../quality-bar-submit.ts", import.meta.url),
);
const submitRuntimePath = fileURLToPath(
  new URL("../quality-bar-submit-runtime.ts", import.meta.url),
);
...
export function installSubmissionCommand(
  installation: {
    ...
    runtimePath: string;
    tokenPath: string;
    trustedProcessPath: string;
    commandIdentity: { birthtimeMs: number; dev: number; ino: number } | null;
    runtimeIdentity: { birthtimeMs: number; dev: number; ino: number } | null;
    tokenIdentity: { birthtimeMs: number; dev: number; ino: number } | null;
  ...
  const runtimeSource = stripTypeScriptTypes(
    readFileSync(submitRuntimePath, "utf8"),
  )
    .replace(
  ...
  let commandTemporaryIdentity: {
    birthtimeMs: number;
    dev: number;
    ino: number;
  } | null = null;
  let runtimeTemporaryIdentity: {
    birthtimeMs: number;
    dev: number;
    ino: number;
  } | null = null;
  let tokenTemporaryIdentity: {
    birthtimeMs: number;
    dev: number;
    ino: number;
  } | null = null;
  try {
    ...
    const temporaryFiles: Array<
      [string, { birthtimeMs: number; dev: number; ino: number } | null]
    > = [
