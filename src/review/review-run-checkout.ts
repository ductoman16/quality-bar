...
function checkoutFailed(
  cause: unknown,
  code: string = "review_run_checkout_failed_definitive",
  message: string = "Review Run checkout failed definitively",
) {
...
function classifyFilesystemFailure(failure: unknown) {
  const code =
...
async function runGit(
  ...
  try {
    ...
    throw classifyGitFailure(
      failure as Error & { stderr?: string },
      arguments_,
    );
  }
