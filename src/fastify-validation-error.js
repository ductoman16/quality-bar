...
/** @param {Array<{instancePath: string, keyword: string, params?: {missingProperty?: string}, parentSchema?: Record<string, any>, schemaPath: string}>} diagnostics */
function declaredError(diagnostics) {
...
/** @param {import("fastify").FastifyRequest} request @param {Parameters<typeof declaredError>[0]} diagnostics */
export function canonicalFastifyValidationError(request, diagnostics) {
  const operation = request.routeOptions.schema?.operationId ?? "";
  const configured =
    /** @type {any} */ (request.routeOptions.schema)?.[ERROR_KEY] ?? MALFORMED;
