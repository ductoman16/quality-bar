...
import { browserView, safeInternalDestination } from "./browser-navigation.ts";
import { browserDocument } from "./browser-ui.ts";
import { BROWSER_CSRF_COOKIE_NAME } from "./browser-session.ts";
import { requireCodedError } from "./coded-error.ts";
import { writeError, writeHtml, writeHtmlDocument } from "./http-response.ts";
...
export function createBrowserPageRoute({
  ...
  browserSessions: ReturnType<
    typeof import("./browser-session.ts").createBrowserSessionService
  >;
  ...
  return function handleBrowserPage(
