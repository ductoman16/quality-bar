import { DurableCoreError } from "../durable/durable-error.js";
import { readCodexExecutionConcurrency } from "./codex-execution-concurrency.js";
import { requireCodexProcessIdentity } from "./codex-process-identity.js";
 ...
 * @param {{fencingToken: number, workerId: string, workId: string, workKind: "review_run" | "waiver_adjudication"}} claim
 * @param {string} codexCliVersion
 * @param {number | undefined} processGroupId
 * @param {{claimLost: () => never, now: () => number, readProcessIdentity: (processId: number) => unknown}} dependencies
 */
export function startCodexExecution(
  ...
  codexCliVersion,
  processGroupId,
  { claimLost, now, readProcessIdentity },
) {
  ...
  const started = durableCore.transaction((/** @type {any} */ transaction) => {
