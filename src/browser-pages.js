...
export function loginPage(intendedDestination) {
  return `<main class="qb-login"><div class="qb-login__card"><div class="qb-login__brand"><span class="qb-brand">QB</span><span class="qb-login__title">Quality Bar</span></div><form class="qb-login__form" id="login-form"><label for="password">Password</label><input autocomplete="current-password" id="password" name="password" required type="password"><button class="qb-btn qb-btn--primary" title="Log in" type="submit">Log in</button><p hidden id="error" role="alert"></p></form></div><style>.qb-login{min-height:100vh;display:grid;place-items:center;padding:24px}.qb-login__card{width:100%;max-width:340px;display:grid;gap:26px}.qb-login__brand{display:flex;align-items:center;justify-content:center;gap:12px}.qb-login__title{font-family:var(--qb-display);font-size:20px;font-weight:650;letter-spacing:-.01em}.qb-login__form{grid-template-columns:1fr;gap:10px}.qb-login__form label{justify-self:start}.qb-login__form input,.qb-login__form button{width:100%}.qb-login__form>p{margin:6px 0 0;color:var(--qb-muted-ink);font-size:13px}</style></main><script id="browser-configuration" type="application/json">${browserConfiguration({ intendedDestination })}</script><script src="/assets/login.js"></script>`;
}
