...
export function completeEvaluationIfTerminal(
  ...
  if (/** @type {number} */ (nonterminalReviewRunCount) > 0) {
    return false;
  }
  ...
  const triggeredImpacts = transaction
    ...
    .map((row) => /** @type {string} */ (row?.impact));
  const outcome = aggregateEvaluationOutcome({
    applicabilityErrorCount: /** @type {number} */ (applicabilityErrorCount),
    criterionErrorCount: /** @type {number} */ (criterionErrorCount),
    failedOrCancelledReviewRunCount: /** @type {number} */ (
      failedOrCancelledReviewRunCount
    ),
    triggeredImpacts,
  });
  transaction.run(
