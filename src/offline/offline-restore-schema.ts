...
import { initializeOrValidateSchema } from "../durable/durable-schema.ts";

function normalizedSql(sql: unknown) {
...
function tableDefinitions(sql: string) {
...
function foreignKeys(database: DatabaseSync, tableName: string) {
  const groups: Map<unknown, Array<Record<string, any>>> = new Map();
...
function indexes(database: DatabaseSync, tableName: string) {
  return database
...
export function schemaDefinition(database: DatabaseSync) {
