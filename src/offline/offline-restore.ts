...
import { verifyRestoredInstallationKey } from "../installation-configuration.ts";
import {
  copyRestoreCandidate,
  validateRestoreCandidate,
  validateRestoreSnapshot,
} from "./offline-restore-snapshot.ts";
import { failRestore, failRetainedRestore } from "./offline-restore-error.ts";
import { validateRestoredCredentials } from "./offline-restore-state.ts";
import { replaceRestoredOperatorAuthority } from "../operator/operator-password.ts";
import { requireRestoredDiscoveryBaseline } from "../restored-discovery.ts";

function synchronize(path: string) {
...
function consolidateStoppedDatabase(databasePath: string) {
