...
export function readEffectiveWaiverOutcome(durableCore, evaluationId) {
  ...
  if (row.adjudication_count === 0) {
    return /** @type {string} */ (row.result_outcome ?? "pending");
  }
  return effectiveEvaluationOutcome({
    activeAdjudicationCount: /** @type {number} */ (row.active_count),
    blockingFindingCount: /** @type {number} */ (row.blocking_count),
    currentWaiverErrorCount: /** @type {number} */ (row.error_count),
    resultOutcome: /** @type {string} */ (row.result_outcome ?? "pending"),
    unwaivedAdvisoryFindingCount: /** @type {number} */ (row.unwaived_count),
  });
}
