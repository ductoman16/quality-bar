import { claimForgejoPollingGeneration } from "../forgejo/forgejo-polling.ts";
import { claimGitHubPollingGeneration } from "../github/github-polling-generation.ts";
import { fail } from "./repository-validation.ts";

export type RepositoryRemovalCore = {
  all(
    sql: string,
...
export function removeNeverUsedRepository(
  durableCore: RepositoryRemovalCore,
  id: string,
) {
...
export function retireUsedRepository(
  durableCore: RepositoryRemovalCore,
  id: string,
  expected: { lifecycle: string; lifecycleRevision: number },
) {
  const [repository] = durableCore.all(
...
function advanceRepositoryPollingGenerations(
  transaction: any,
  repositoryId: string,
) {
