import { readCommitDeliverySources } from "./system-delivery-lineage.js";

/** @param {any} durableCore @param {"github" | "forgejo"} provider */
function rejectOrphanedDeliveryPublications(durableCore, provider) {
  const checks = [
...
/** @param {any} durableCore @param {"github" | "forgejo"} provider */
function rejectOrphanedDeliveryAttempts(durableCore, provider) {
  ...
  const aggregateSources = new Set([
    ...durableCore
      ...
      .map(/** @param {any} row */ (row) => row.evaluation_id),
    ...durableCore
      .all(
  ...
  const inlineSources = new Set([
    ...durableCore
      ...
      .map(/** @param {any} row */ (row) => row.finding_id),
    ...durableCore
      .all(
...
/** @param {any} durableCore @param {"github" | "forgejo"} provider @returns {any[]} */
export function readSystemDeliveryRows(durableCore, provider) {
