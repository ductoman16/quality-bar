import { readForgejoPollingFailure } from "../forgejo/forgejo-polling-read.ts";
import { readGitHubPollingFailure } from "../github/github-polling-read.ts";
import {
...
} from "./system-polling-fact-support.ts";

export function readPollingProvider(
  durableCore: any,
  ...
  validatePollingGateOwnership(
    durableCore,
    provider,
    new Set(rows.map((row: any) => row.connection_id)),
  );
  ...
  for (const connection of connections) {
    ...
    const connectionRow = rows.find(
      (row: any) => row.connection_id === connection.connection_id,
    );
    ...
    if (
      ...
      connection._next_attempt_at_millis = minMillis(
        connection.repositories
          .map((repository: any) => repository._next_attempt_at_millis)
      ...
      const eligibleRepositories = connection.repositories.filter(
        (repository: any) => repository._eligible_for_attempt,
      );
      connection._next_attempt_after_correction =
        connection._next_attempt_at_millis === null &&
        eligibleRepositories.length > 0 &&
        eligibleRepositories.every(
          (repository: any) => repository._next_attempt_after_correction,
        );
      connection.next_attempt_after_correction =
        connection._next_attempt_after_correction;
    }
