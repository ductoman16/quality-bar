import { readCodexExecutionConcurrency } from "../codex/codex-execution-concurrency.ts";
...
function requireExecutionRow(row: any) {
  if (
    !row ||
...
function requireFailureRow(row: any) {
  if (
    !row ||
    !["review_run", "waiver_adjudication"].includes(row.work_kind) ||
...
function leaseStatus(row: any, status: "queued" | "running", now: number) {
  if (status === "running") {
    return "running";
  }
  if (row.worker_id === null) {
...
export function readSystemCodexExecutionFacts(
  durableCore: { all: (sql: string) => any[]; get: (sql: string) => any },
  {
    codex,
    now,
    storage,
