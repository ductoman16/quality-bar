import { forgejoDefinitiveFailureScope } from "../forgejo/forgejo-failure.js";
import { forgejoPullRequestSnapshot } from "../forgejo/forgejo-automatic-evaluation.js";
import { pullRequestSnapshot } from "../github/github-pull-request-snapshot.js";
import {
...
export function maxMillis(left, right) {
  if (left === null) {
    return right;
  }
  if (right === null) {
...
export function minMillis(values) {
  return values.length === 0 ? null : Math.min(...values);
}

/** @param {number | null} value @returns {{millis: number | null, afterCorrection: boolean}} */
export function nextAttemptState(value) {
  if (value === null) {
...
export function nextAttemptMillis(value) {
  return nextAttemptState(value).millis;
}

/** @param {{millis: number | null, afterCorrection: boolean}} left @param {{millis: number | null, afterCorrection: boolean}} right */
export function maxAttemptState(left, right) {
...
/** @param {any} row @param {"github" | "forgejo"} provider */
export function readPollingSnapshot(row, provider) {
...
/** @param {any} durableCore @param {string} connectionId @param {number} verifiedAt @param {any} pollingFailure */
...
export function rejectOrphanedPollingRows(durableCore, provider) {
...
/** @param {any} durableCore @param {"github" | "forgejo"} provider @param {Set<string>} connectionIds */
...
/** @param {any} row @param {"github" | "forgejo"} provider */
export function pollingRepository(row, provider) {
  if (
