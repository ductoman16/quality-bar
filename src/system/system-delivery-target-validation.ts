import { githubFeedbackSourceIdentity } from "../github/github-feedback-identity.ts";

const COMMIT_STATES = new Set(["pending", "success", "failure", "error"]);

function invalid(provider: string, surface: string) {
  throw new TypeError(
    `${provider} System ${surface} delivery target is invalid`,
  );
}

function objectTarget(value: any, provider: string, surface: string) {
...
function keys(target: Record<string, any>) {
  return Object.keys(target).sort().join(",");
}

function repositoryMatches(value: unknown, expected: any) {
  return (
...
function positiveNumberMatches(value: unknown, expected: number) {
  return (
    typeof value === "number" &&
...
function hasIdentity(
  body: unknown,
  label: "Adjudication" | "Evaluation" | "Finding",
  id: string,
) {
  return githubFeedbackSourceIdentity(body, label) === id;
}

function evaluationUrlMatches(value: unknown, evaluationId: string) {
...
function validateAggregateTarget(
  target: Record<string, any>,
  expected: any,
  provider: string,
  surface: string,
...
export function validateSystemDeliveryTargets(
  durableCore: any,
  ...
  const evaluations = new Map<any, any>(
    durableCore
      .all(
      ...
      .map((row: any) => [row.evaluation_id, row]),
  );
  const automatic = new Map<any, any>(
    durableCore
      .all(
  ...
  const findings = new Map<any, any>(
    durableCore
      .all(
      ...
      .map((row: any) => [row.finding_id, row]),
  );
  const adjudications = new Map<any, any>(
    durableCore
      .all(
      ...
      .map((row: any) => [row.waiver_adjudication_id, row]),
  );
  const decisions = new Map<any, any>(
    durableCore
      ...
      .map((row: any) => [row.waiver_decision_id, row]),
  );
