...
import { failBackup, owningBackupError } from "./sqlite-backup-error.ts";
import {
  readValidatedBackups,
  retainLatestValidatedBackups,
} from "./validated-backup.ts";

export { SqliteBackupError } from "./sqlite-backup-error.ts";

export function installationKeyIdentity(masterKey: Buffer) {
...
function backupTimestamp(timestamp: number) {
...
export async function createValidatedBackup({
  ...
  let validatedDatabase: DatabaseSync | undefined;
  ...
  try {
    ...
    try {
      ...
      try {
        integrity = (
          validatedDatabase.prepare("PRAGMA integrity_check").get() as
