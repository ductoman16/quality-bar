...
} from "../execution-analytics.ts";
import { evaluationOutcome } from "./analytics-evaluation-outcome.ts";
import {
  derivePullRequestCriterionTransitions,
  validatedAnalyticsFilters,
} from "./analytics-filter.ts";
import { readAnalyticsFactRows } from "./analytics-query.ts";

export { AnalyticsError } from "../execution-analytics.ts";
...
export function buildDailyTrend(
  rows: Array<Record<string, import("node:sqlite").SQLInputValue>>,
) {
  ...
  for (const row of rows) {
    ...
    const outcome = row.terminal_outcome as
      | "advisory"
      | "blocking"
      | "clear"
      | "error"
...
export function createAnalyticsService(
  ...
  return {
    read(inputFilters?: Record<string, unknown>) {
      const filters = validatedAnalyticsFilters(inputFilters);
      const overviewWindow = {
        end: filters.end === undefined ? now() : (filters.end as number),
        start: filters.start === undefined ? 0 : (filters.start as number),
      };
      ...
      try {
        ...
        for (const row of findingRows) {
          ...
          findingCounts[row?.finding_impact as "advisory" | "blocking"] += 1;
        }
        let requestedFindings = 0;
        let waivedFindings = 0;
        for (const row of waiverRows) {
          if (
            ![0, 1].includes(row?.has_waiver_request as number) ||
            ![0, 1].includes(row?.has_accepted_decision as number)
          ) {
            throw new AnalyticsError(
              "analytics_fact_invalid",
              "Canonical analytics fact is invalid",
          ...
          requestedFindings += row?.has_waiver_request as number;
          waivedFindings += row?.has_accepted_decision as number;
        }
        ...
        const factsForRun = (
          ...
          >,
          reviewRunId: import("node:sqlite").SQLInputValue | undefined,
        ...
        return {
          ...
          pull_request_criterion_transitions:
            derivePullRequestCriterionTransitions(
              progressionRows as Record<string, unknown>[],
              selectedEvaluationIds,
              selectedCriterionScopes,
            ),
