...
} from "./analytics-query-window.ts";
import { EVALUATION_WAIVER_SELECTION } from "../evaluation/evaluation-waiver-selection.ts";
import { readAutomaticEvaluationProgressionRows } from "./analytics-automatic-progression-query.ts";
...
export function readAnalyticsFactRows(
  ...
  evaluationOverviewWindow: { end: number; start: number },
) {
  ...
  const outcomeRows = evaluationRows.map((row) => ({
    ...row,
  ...
  const selectedEvaluationIds = filteredEvaluationIds(
    ...
    filterFactRows as Record<string, unknown>[],
    filters,
  );
  ...
  const inReviewScope = (
    row: Record<string, import("node:sqlite").SQLInputValue> | undefined,
    scopes: Set<string> = reviewScopes,
    evaluationIds: Set<string> = selectedEvaluationIds,
  ) =>
    !hasFilters ||
  ...
  const inCriterionScope = (
    row: Record<string, import("node:sqlite").SQLInputValue> | undefined,
    scopes: Set<string> = criterionScopes,
    evaluationIds: Set<string> = selectedEvaluationIds,
  ) =>
    !hasFilters ||
    (typeof row?.evaluation_id === "string" &&
  ...
  const filteredEvaluationRows = hasFilters
    ? outcomeRows.filter((row) =>
        selectedEvaluationIds.has(row.analytics_evaluation_id as string),
      )
    : outcomeRows;
