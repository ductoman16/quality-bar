 ...
 *
 * @param {{
 *   durableCore: { retentionTransaction: (callback: (transaction: { run: (sql: string, ...parameters: any[]) => any }) => any) => any },
 *   now: () => number
 * }} options
 */
export function cleanupEligibleRetentionData({ durableCore, now }) {
  ...
  return durableCore.retentionTransaction((transaction) => {
    const changes = /** @type {Record<string, { changes: number }>} */ ({});
