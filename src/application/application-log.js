...
export function createApplicationSecretRegistry() {
  /** @type {string[]} */
  const knownSecrets = [];
  const registerSecret = (/** @type {string} */ secret) => {
...
/** @param {unknown} value @param {string} field */
function optionalOperationIdentifier(value, field) {
  if (value === undefined || value === null) {
    return undefined;
  }
...
/** @param {string} value @param {string[]} knownSecrets */
function redactCredentialShapes(value, knownSecrets) {
  let redacted = value;
...
/** @param {unknown} value @param {string} field @param {string[]} knownSecrets */
function safeDetail(value, field, knownSecrets) {
  if (value === undefined || value === null) {
    return undefined;
  }
...
/** @param {string} value @param {{knownSecrets?: string[]}} [options] */
export function redactOrdinaryDetail(value, { knownSecrets = [] } = {}) {
  if (
    typeof value !== "string" ||
...
/** @param {string} line @param {string[]} knownSecrets */
function normalizeStructuredLogLine(line, knownSecrets) {
...
/** @param {string} line @param {{knownSecrets?: string[]}} [options] */
export function sanitizeStructuredLogLine(line, { knownSecrets = [] } = {}) {
  if (
    !Array.isArray(knownSecrets) ||
    !knownSecrets.every(
      (secret) => typeof secret === "string" && secret.length > 0,
 ...
 *   readDurableCore: () => { run: (sql: string, ...parameters: any[]) => unknown } | null,
 *   knownSecrets?: string[]
 * }} options
 */
...
/** @param {(line: string) => unknown} hostWriter @param {() => any} readDurableCore @param {string[]} [knownSecrets] */
