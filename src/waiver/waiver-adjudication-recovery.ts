...
import { assertWaiverRecoveryRepositoryAvailable } from "./waiver-recovery-admission.ts";

export function classifyWaiverAdjudicationRecovery(adjudication: any) {
  if (
    !adjudication ||
...
function readAdjudication(access: any, adjudicationId: string) {
  return access.get(
...
function readRequests(access: any, adjudicationId: string) {
  return access.all(
...
function assertLatestUndecidedRecoverySource(
  transaction: any,
...
function recoveryResource(source: any, requests: any[]) {
  return {
    adjudication: {
...
function persistReplay(transaction: any, input: any) {
  transaction.run(
    ...
    recover({
      ...
      return durableCore.transaction((transaction: any) => {
        ...
        const resource = queueWaiverAdjudication(transaction, {
          ...
          requestIds: requests.map((request: any) => request.id),
          writeRequests: () =>
