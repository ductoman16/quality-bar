...
function latestAttempt(row: any) {
...
export function readEvaluationWaiverAdjudications(
  durableCore: any,
  evaluationId: string,
) {
  return durableCore
    ...
    .map((row: any) => {
      ...
      const requestIds = durableCore
        ...
        .map((request: any) => request.waiver_request_id);
        ...
        followup:
          row.followup_status === null
            ...
            : {
                ...
                local: localFollowups.map((local: any) => ({
