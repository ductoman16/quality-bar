...
import { createWaiverErrorRetryService } from "./waiver-error-retry.ts";
import { createWaiverAdjudicationRecoveryService } from "./waiver-adjudication-recovery.ts";
import { waiverRequestNextAction } from "./waiver-request-lifecycle.ts";

export { canonicalWaiverErrorRetryRequest } from "./waiver-error-retry.ts";
...
export function canonicalWaiverBatchRequest(candidate: unknown) {
  ...
  const seen = new Set();
  const requests = input.requests.map((request: any) => {
    if (
      !request ||
  ...
  requests.sort((left: { finding_id: string }, right: { finding_id: string }) =>
      ...
      return durableCore.transaction((transaction: any) => {
        ...
        const findings = transaction.all(
          ...
          ...canonical.requests.map(
            ({ finding_id }: { finding_id: string }) => finding_id,
          ),
        );
        const byId = new Map<any, any>(
          findings.map((finding: any) => [finding.id, finding]),
        );
        const priorRationales = new Set(
          transaction
            ...
            .map((prior: any) => `${prior.finding_id}\u0000${prior.rationale}`),
        );
        ...
        return persistQueuedWaiverAdjudication(transaction, {
          ...
          route,
          writeRequests: () =>
            canonical.requests.map((requestValue: any, index: number) => {
