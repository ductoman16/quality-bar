...
export function readWaiverRequest(row: Record<string, any>) {
  if (
    !row ||
...
export function readWaiverDecision(row: Record<string, any>) {
  if (
    !row ||
    typeof row.id !== "string" ||
...
export function readWaiverAdjudication(
  access: { all(sql: string, ...parameters: any[]): Record<string, any>[] },
...
export function createWaiverResourceReader(durableCore: any) {
  function readAdjudication(adjudicationId: string) {
    ...
    const requestIds = durableCore
      ...
      .map(({ waiver_request_id: id }: { waiver_request_id: string }) => id);
