...
} from "./forgejo-repository.ts";
import { createForgejoPullRequestReader } from "./forgejo-pull-request-reader.ts";
import { createForgejoPublisher } from "./forgejo-publisher.ts";
import { normalizedForgejoBaseUrl } from "./forgejo-url.ts";
import { forgejoResponseJson } from "./forgejo-response-failure.ts";
import {
  currentIoOperationSignal,
  throwIfIoOperationAborted,
} from "../io-operation-context.ts";

export { normalizedForgejoBaseUrl } from "./forgejo-url.ts";
...
function routeArray(value: unknown, route: string, field: string) {
  if (
    !Array.isArray(value) ||
...
export function createForgejoVerifier({
  ...
  return {
    ...
    async verify({
      ...
      const get = async (path: string, repositoryId?: number) => {
        signal?.throwIfAborted();
        let response;
        try {
      ...
      const enumerated: ReturnType<typeof repository>[] = [];
      const enumeratedIds = new Set();
      let verifiedPrincipal: { id: number; login: string } | undefined;
      ...
      const repositoryChecks: any[] = selected.map((candidate) => ({
      ...
      for (const [index, selectedCandidate] of selected.entries()) {
        const selectedRepository = selectedCandidate as NonNullable<
