import {
  attemptForgejoDelivery,
  recordForgejoDeliveryHealth,
} from "./forgejo-delivery-service.ts";
import { createIoDutyScheduler } from "../io-execution-pool.ts";
import {
  formatWaiverAdjudicationFollowup,
  formatWaiverDecisionFollowup,
} from "../waiver/waiver-followup.ts";

const PUBLICATION_INTERVAL_MS = 1_000;

function target(serialized: string, expected: any) {
  let parsed;
  try {
    parsed = JSON.parse(serialized);
  } catch {
    throw new TypeError("Forgejo waiver follow-up delivery target is invalid");
  }
  if (JSON.stringify(parsed) !== JSON.stringify(expected)) {
    throw new TypeError("Forgejo waiver follow-up delivery target is invalid");
  }
  return parsed;
}

export function createForgejoWaiverFollowupService(
  durableCore: any,
  { cipher, externalOrigin, ioPool, now = () => Date.now(), verifier }: any,
) {
  if (
    typeof durableCore?.all !== "function" ||
    typeof durableCore?.get !== "function" ||
    typeof durableCore?.transaction !== "function" ||
    typeof cipher?.decrypt !== "function" ||
    typeof ioPool?.run !== "function" ||
    typeof now !== "function" ||
    typeof verifier?.publishAggregateFeedback !== "function" ||
    typeof verifier?.reconcileAggregateFeedback !== "function" ||
    typeof verifier?.publishInlineFeedback !== "function" ||
    typeof verifier?.reconcileInlineFeedback !== "function"
  ) {
    throw new TypeError("Forgejo waiver follow-up dependencies are invalid");
  }
  const origin = new URL(externalOrigin);
  if (!["http:", "https:"].includes(origin.protocol)) {
    throw new TypeError("Forgejo waiver follow-up requires an HTTP origin");
  }
  let timer: ReturnType<typeof setInterval> | null = null;
  let running = false;

  function detailsUrl(evaluationId: string) {
    const url = new URL("/", origin);
    url.searchParams.set("view", "evaluations");
    url.searchParams.set("evaluation_id", evaluationId);
    return url.toString();
  }

  async function publishWaiting() {
    if (running) {
      return;
    }
    running = true;
    try {
      const rows = durableCore.all(
        `SELECT followups.waiver_adjudication_id, followups.evaluation_id,
                followups.outcome,
                adjudications.base_commit, adjudications.head_commit,
                automatic.pull_request_number,
                automatic.repository_id,
                repositories.forge_repository_id, repositories.name,
                connections.id AS connection_id, connections.base_url,
                credentials.encrypted_credential
         FROM forgejo_waiver_adjudication_followups AS followups
         JOIN waiver_adjudications AS adjudications
           ON adjudications.id = followups.waiver_adjudication_id
         JOIN forgejo_automatic_evaluations AS automatic
           ON automatic.evaluation_id = followups.evaluation_id
         JOIN forgejo_repositories AS repositories
           ON repositories.repository_id = automatic.repository_id
         JOIN forgejo_connections AS connections
           ON connections.id = repositories.connection_id
         JOIN forgejo_connection_credentials AS credentials
           ON credentials.connection_id = connections.id
         WHERE (followups.publication_status = 'waiting'
            OR EXISTS (SELECT 1 FROM forgejo_waiver_decision_followups AS local
                       WHERE local.waiver_adjudication_id = followups.waiver_adjudication_id
                         AND local.publication_status = 'waiting'))
           AND connections.lifecycle = 'enabled'
         ORDER BY adjudications.rowid`,
      );
      for (const row of rows) {
        const repository = {
          full_name: row.name as string,
          id: row.forge_repository_id as number,
        };
        const authentication = () => ({
          base_url: row.base_url,
          token: cipher.decrypt(row.connection_id, row.encrypted_credential),
        });
        const identity = {
          adjudication_id: row.waiver_adjudication_id,
          base_commit: row.base_commit,
          details_url: detailsUrl(row.evaluation_id),
          evaluation_id: row.evaluation_id,
          head_commit: row.head_commit,
          outcome: row.outcome,
        };
        const decisions = durableCore.all(
          `SELECT decisions.outcome, decisions.explanation,
                  decisions.error_code, decisions.error_detail,
                  decisions.waiver_request_id AS request_id,
                  requests.finding_id
           FROM waiver_decisions AS decisions
           JOIN waiver_requests AS requests ON requests.id = decisions.waiver_request_id
           WHERE decisions.waiver_adjudication_id = ? ORDER BY decisions.rowid`,
          row.waiver_adjudication_id,
        );
        const aggregateBody = formatWaiverAdjudicationFollowup(
          identity,
          decisions,
        );
        const aggregateTarget = {
          body: aggregateBody,
          pull_request_number: row.pull_request_number,
          repository_id: row.forge_repository_id,
        };
        const aggregate = durableCore.get(
          "SELECT publication_status FROM forgejo_waiver_adjudication_followups WHERE waiver_adjudication_id = ?",
          row.waiver_adjudication_id,
        );
        if (aggregate?.publication_status === "waiting") {
          const deliver = (method: any, serialized: string) => {
            const input = target(serialized, aggregateTarget);
            return method(
              authentication(),
              repository,
              input.pull_request_number,
              input.body,
            );
          };
          await attemptForgejoDelivery(durableCore, {
            connectionId: row.connection_id,
            create: (serialized) =>
              deliver(verifier.publishAggregateFeedback, serialized),
            reconcile: (serialized) =>
              deliver(verifier.reconcileAggregateFeedback, serialized),
            now,
            repositoryId: row.repository_id,
            sourceId: `waiver-adjudication:${row.waiver_adjudication_id}`,
            surface: "aggregate_feedback",
            target: JSON.stringify(aggregateTarget),
            onDefinitive(transaction, failure, attemptedAt) {
              transaction.run(
                "UPDATE forgejo_waiver_adjudication_followups SET publication_status = 'unavailable', error_code = ?, error_detail = ? WHERE waiver_adjudication_id = ? AND publication_status = 'waiting'",
                failure.code,
                failure.detail,
                row.waiver_adjudication_id,
              );
              recordForgejoDeliveryHealth(
                transaction,
                row.connection_id,
                attemptedAt,
                failure,
              );
            },
            onSuccess(transaction, externalId, publishedAt) {
              transaction.run(
                "UPDATE forgejo_waiver_adjudication_followups SET publication_status = 'succeeded', external_id = ?, published_at = ? WHERE waiver_adjudication_id = ? AND publication_status = 'waiting'",
                externalId,
                publishedAt,
                row.waiver_adjudication_id,
              );
            },
          });
        }
        const locals = durableCore.all(
          `SELECT local.waiver_decision_id, local.path, local.side,
                  local.start_line, local.start_side, local.line,
                  decisions.outcome, decisions.explanation,
                  decisions.waiver_request_id AS request_id, requests.finding_id
           FROM forgejo_waiver_decision_followups AS local
           JOIN waiver_decisions AS decisions ON decisions.id = local.waiver_decision_id
           JOIN waiver_requests AS requests ON requests.id = decisions.waiver_request_id
           WHERE local.waiver_adjudication_id = ? AND local.publication_status = 'waiting'
           ORDER BY decisions.rowid`,
          row.waiver_adjudication_id,
        );
        for (const local of locals) {
          const comment = {
            body: formatWaiverDecisionFollowup(identity, local),
            commit_id: row.head_commit,
            line: local.line,
            path: local.path,
            side: local.side,
            ...(local.start_line === null
              ? {}
              : { start_line: local.start_line, start_side: local.start_side }),
          };
          const inlineTarget = {
            ...comment,
            pull_request_number: row.pull_request_number,
            repository_id: row.forge_repository_id,
          };
          const deliver = (method: any, serialized: string) => {
            const input = target(serialized, inlineTarget);
            const {
              pull_request_number,
              repository_id: ignored,
              ...storedComment
            } = input;
            void ignored;
            return method(
              authentication(),
              repository,
              pull_request_number,
              storedComment,
            );
          };
          await attemptForgejoDelivery(durableCore, {
            connectionId: row.connection_id,
            create: (serialized) =>
              deliver(verifier.publishInlineFeedback, serialized),
            reconcile: (serialized) =>
              deliver(verifier.reconcileInlineFeedback, serialized),
            now,
            repositoryId: row.repository_id,
            sourceId: `waiver-decision:${local.waiver_decision_id}:${local.finding_id}`,
            surface: "inline_feedback",
            target: JSON.stringify(inlineTarget),
            onDefinitive(transaction, failure, attemptedAt) {
              transaction.run(
                "UPDATE forgejo_waiver_decision_followups SET publication_status = 'unavailable', error_code = ?, error_detail = ? WHERE waiver_decision_id = ? AND publication_status = 'waiting'",
                failure.code,
                failure.detail,
                local.waiver_decision_id,
              );
              recordForgejoDeliveryHealth(
                transaction,
                row.connection_id,
                attemptedAt,
                failure,
              );
            },
            onSuccess(transaction, externalId, publishedAt) {
              transaction.run(
                "UPDATE forgejo_waiver_decision_followups SET publication_status = 'succeeded', external_id = ?, published_at = ? WHERE waiver_decision_id = ? AND publication_status = 'waiting'",
                externalId,
                publishedAt,
                local.waiver_decision_id,
              );
            },
          });
        }
      }
    } finally {
      running = false;
    }
  }
  const schedule = createIoDutyScheduler(ioPool, "delivery", publishWaiting);
  return {
    destroy() {
      if (timer) {
        clearInterval(timer);
      }
      timer = null;
      schedule.cancel();
    },
    publishWaiting,
    start() {
      if (timer) {
        return;
      }
      schedule.background();
      timer = setInterval(schedule.background, PUBLICATION_INTERVAL_MS);
      timer.unref?.();
    },
  };
}
