import { verifiedForgejoRepositories } from "./forgejo-connection-rotation.ts";
import { verifiedForgejoRepositoryEvidence } from "./forgejo-repository-check.ts";
import {
...
export function failedForgejoReactivationVerification(
  error: Error & { verificationEvidence?: unknown },
  repositoryIds: number[],
) {
  ...
  const result =
    ...
    typeof error.verificationEvidence === "object"
      ? (error.verificationEvidence as any)
      : null;
  const capabilityEntries =
    result?.capabilities &&
    !Array.isArray(result.capabilities) &&
