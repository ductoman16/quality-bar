import { createEvaluationCollection } from "./evaluation-collection.ts";
import { EVALUATION_SELECTION, readEvaluation } from "./evaluation-resource.ts";
import { readEvaluationMonitors } from "./evaluation-monitor.ts";
import { failEvaluation } from "./evaluation-validation.ts";

export function createEvaluationCollectionReader(
  durableCore: { all: Function; get: Function },
  masterKey: Buffer,
) {
  ...
  function serialize(
    rows: (Record<string, import("node:sqlite").SQLInputValue> | undefined)[],
  ) {
