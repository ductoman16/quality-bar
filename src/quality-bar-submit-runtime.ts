...
export function outerProcessId(status: string, fallback: number) {
...
function readBoundedText(descriptor: number, maxBytes: number) {
  const chunks = [];
  let length = 0;
  while (true) {
...
export function removeOwnedFile(
  path: string,
  expected: { dev: number; ino: number; birthtimeMs?: number } | null,
) {
  if (!expected) {
    return;
  }
...
export function readSubmissionText(input: string | undefined) {
...
export function readResponse(path: string) {
  const pathStatus = fs.lstatSync(path);
  const descriptor = fs.openSync(
    path,
    fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW | fs.constants.O_NONBLOCK,
