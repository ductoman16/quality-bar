...
} from "../src/codex/codex-execution-claim.ts";
...
test("renewal loss preserves the exact owner error for both work kinds", () => {
  for (const [workKind, expectedCode, expectedMessage] of [
    ...
    const losses: { code?: string; message?: string }[] = [];
    service.startRenewal(
      {
        ...
        workId: `${workKind}-1`,
        workKind: workKind as "review_run" | "waiver_adjudication",
      },
      (error) => losses.push(error as { code?: string; message?: string }),
    );
    renewal();
    assert.deepEqual(
