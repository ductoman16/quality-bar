...
import {
  ...
  executeReviewRun,
} from "../src/review/review-run-execution.ts";
import { ReviewRunCheckoutError } from "../src/review/review-run-checkout.ts";
...
test("builds only the fixed Review Run contract and frozen evidence boundaries into the prompt", () => {
  ...
  assert.throws(
    () => createReviewRunPrompt({ ...run, fileChanges: undefined } as any),
...
test("cleanup failure cannot replace the exact owning execution failure", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal((error as any).checkoutCleanupFailure, cleanupFailure);
      return true;
    },
  );
});
...
test("an unexpected started failure has one stable safe owning detail", async () => {
  ...
  let persistedFailure: ReviewRunExecutionError | undefined;
  await assert.rejects(
    () =>
      executeReviewRun(durableCore(), claim, {
        ioPool: createIoExecutionPool(),
