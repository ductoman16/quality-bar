export function repositoryEvidence(id: number, name: string) {
  return {
...
export function pullRequest(
  number: number,
  overrides: Partial<{ draft: boolean; head: { sha: string } }> = {},
) {
  return {
...
export function forgejoVerification(repositories: any[]) {
  return {
    capabilities: {
...
export function enabledRepositoryPoll(core: any, fields: string) {
  return core.get(
    `SELECT ${fields}
       FROM repositories
...
export function createOneShotForgejoBaselineFailure(
  ...
  return {
    ...
    fails(repositoryId: number, baseUrl: string) {
