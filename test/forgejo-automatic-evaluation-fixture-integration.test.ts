...
import { acquireForgejoAutomaticEvaluations } from "../src/forgejo/forgejo-automatic-evaluation-admission.ts";
import { newlyEligibleForgejoPullRequests } from "../src/forgejo/forgejo-automatic-evaluation.ts";
import { ApplicationShutdownError } from "../src/application/application-shutdown.ts";
...
test("Forgejo fixture turns a newly ready provider snapshot into acquisition input", async () => {
  ...
  const acquisitions: { pullRequest: any; repositoryId: string }[] = [];
...
test("a Forgejo acquisition failure does not skip a later eligible Changeset", async () => {
  ...
  assert.equal(
    (acquired.failures[0] as any)?.code,
