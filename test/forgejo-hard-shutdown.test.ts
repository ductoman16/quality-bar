...
function forgejoVerificationBody(path: string) {
...
function beginVerification(verifier: ReturnType<typeof createForgejoVerifier>) {
...
test("Forgejo verification preserves hard shutdown during response parsing", async () => {
  ...
  await assert.rejects(completion, (error: unknown) => error === failure);
});

test("Forgejo verification preserves hard shutdown during private Git read", async () => {
  const gitRead = Promise.withResolvers<void>();
  const { completion, workers } = beginVerification(
    createForgejoVerifier({
      fetch: async (input) => {
