...
async function submit(
  ...
  checkoutPath: string,
  environment: Record<string, string> = channel.environment,
) {
...
test("the generated command executes only with its installed sibling runtime", async (context) => {
  ...
  try {
    ...
    assert.match(
      commandSource,
      /new URL\(\s*"\.\/quality-bar-submit-runtime\.ts"/s,
    );
    assert.match(
      commandSource,
  ...
  const channel = await openReviewRunSubmissionChannel(
    ...
    { prepare() {} },
    {
      ...
      publishFile(temporaryPath, targetPath) {
        if (targetPath.endsWith("quality-bar-submit-runtime.ts")) {
...
const runtimeMutations: Array<[string, (path: string) => void]> = [
