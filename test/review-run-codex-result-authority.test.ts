...
function processThatExits(code: number) {
...
function processThatReportsError(processFailure: Error) {
  const child = new EventEmitter();
  const process = Object.assign(child, {
    pid: 79,
    stderr: new PassThrough(),
...
function committedFailedChannel(failure: Error) {
  return {
    ...acceptedChannel(),
    accepted: () => false,
    failure: () => failure,
    hasCommittedSubmission: () => true,
    hasPendingSubmission: () => true,
    waitForResult: () => Promise.resolve("failed" as "failed"),
  };
}
...
test("a committed Result retains process-error termination failure as a diagnostic", async () => {
  ...
  const result = await runReviewRunCodex({
    ...
    resultService: { prepare() {} },
    run,
    spawnProcess: () => processThatReportsError(processFailure) as any,
  });
  assert.deepEqual(result, { diagnosticFailures: [terminationFailure] });
});
