...
function prepareRequest(core: any, outcome: "accepted" | "denied" | null) {
...
function insertLaterAdjudication(core: any) {
  core.run(
...
for (const outcome of [null, "accepted", "denied"]) {
  test(`a ${outcome ?? "pending"} Request cannot be associated with a later Adjudication`, () => {
    ...
    try {
      ...
      prepareRequest(core, outcome as "accepted" | "denied" | null);
