...
import { stubUnavailableEvaluationService } from "./service-stubs-support.ts";
...
test("the browser creates, replays, polls, and reads complete zero-Review Evaluations", async () => {
  const logs: string[] = [];
  let nextId = 0;
  let evaluationMasterKey: Buffer | undefined;
  ...
  assert.deepEqual(
    ((await collection.json()) as { items: Array<{ id: string }> }).items.map(
      ({ id }) => id,
    ),
    ["evaluation-1"],
  );
