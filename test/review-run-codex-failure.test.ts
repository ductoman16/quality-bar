...
import { REVIEW_RUN_TERMINAL_FAILURE_CODES } from "../src/review/review-run-codex-failure.ts";
...
function processThatReports(event: unknown) {
...
function processThatExitsWithStderr(stderr: string) {
  const child = new EventEmitter();
  const process = Object.assign(child, {
    pid: 78,
    stderr: new PassThrough(),
...
function channel({
  failure = null,
  result,
}: { failure?: Error | null; result?: "failed" } = {}): any {
  return {
    accepted: () => false,
    ...
    await assert.rejects(
      () =>
        runReviewRunCodex({
          ...
          resultService: { prepare() {} },
          run,
          spawnProcess: () => processThatExitsWithStderr(stderr) as any,
        }),
      (error) => {
        assert.ok(error instanceof ReviewRunExecutionError);
        assert.equal(error.code, expectedCode);
    ...
    () =>
      runReviewRunCodex({
        ...
        spawnProcess: () =>
          processThatReports({
            error: {
              message: "Unexpected backend failure for submission token secret",
            },
            type: "turn.failed",
          }) as any,
      }),
...
test("a post-spawn process error waits for close, transcript, and process facts", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal((error.cause as any).processError, processFailure);
      return true;
    },
  );
...
test("a process error closes submission before termination can accept a Result", async () => {
  ...
  let accepted = false;
  let closed = false;
  let acceptSubmission: (value: "accepted") => void = () => {};
