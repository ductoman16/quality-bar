...
/** @param {any} core */
function insertDependentRepositories(core) {
  core.run(
...
test("GitHub App rotation verifies active dependents before one encrypted swap", async (context) => {
  ...
  const service = createAvailableGitHubConnectionService(core, {
    ...
    verifier: /** @type {any} */ ({
      ...
      /** @param {any} credential @param {number} installationId @param {number[] | undefined} repositoryIds */
  ...
  const after = /** @type {{encrypted_credential: string}} */ (
    core.get("SELECT encrypted_credential FROM github_connection_credentials")
  ).encrypted_credential;
  ...
  assert.equal(
    /** @type {{trigger: string, outcome: string, error_code: string}} */ (
      core.get(
  ...
  assert.deepEqual(
    JSON.parse(
      /** @type {{affected_repository_ids: string}} */ (
        core.get(
