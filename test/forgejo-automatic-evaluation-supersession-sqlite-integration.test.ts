...
import { assertSupersessionFencesRunningWorker } from "./automatic-evaluation-supersession-support.ts";
import { availableStorageReserve } from "./storage-reserve-support.ts";

function changeset(base: string, head: string) {
  return {
...
test("a different Forgejo pair supersedes only the target PR's nonterminal work", (context) => {
  ...
  const admit = (
    pair: { base_commit: string; head_commit: string },
    pullRequestNumber: number = 17,
  ) =>
