...
} from "../src/operator/operator-password-bootstrap.ts";
import { OPERATOR_PASSWORD_VERIFIER_METADATA_KEY } from "../src/operator/operator-password.ts";
...
test("the host command has no environment or argument password channel", () => {
  const command = readFileSync(
    new URL("../src/bootstrap-operator-password.ts", import.meta.url),
    "utf8",
  );
