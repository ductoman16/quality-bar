...
} from "../src/owned-artifact-cleanup.ts";

import {
  StorageReserveError,
  createStorageReserveGate,
  requireStorageReservePause,
} from "../src/storage-reserve.ts";
...
test("runtime reserve facts remeasure both owned filesystems for every guarded action", () => {
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assert.equal((error as any).action, "codex_start");
      assert.deepEqual((error as any).facts, {
        filesystems: [
          {
            available_bytes: 8 * GIB,
            filesystem: "state",
...
test("a failed runtime measurement owns an exact filesystem error without inferred health", () => {
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assert.equal((error as any).action, "polling_observation_advancement");
      assert.equal((error as any).filesystem, "checkouts");
      assert.equal((error as any).path, "/checkouts");
...
function temporaryRoot(context: import("node:test").TestContext) {
  const root = join(
    tmpdir(),
...
function cleanupDurableCore(rows: Record<string, unknown>[]) {
  return { all: () => rows };
}
