...
import { createAnalyticsService } from "../src/analytics/analytics.ts";
import { derivePullRequestCriterionTransitions } from "../src/analytics/analytics-filter.ts";
import { evaluation, progression } from "./analytics-filter-support.ts";
...
test("Analytics keeps triggered transition outcomes distinct", () => {
  const cases: Array<
    [
      string,
      "triggered_to_clear" | "triggered_to_error" | "no_longer_applicable",
    ]
  > = [
...
test("Waiver activity uses its event clock without dropping an older Evaluation", () => {
  const queriedWindows: import("node:sqlite").SQLInputValue[][] = [];
