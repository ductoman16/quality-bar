...
test("Forgejo Connection HTTP registration keeps PAT input write-only and preserves its owning error", async () => {
  ...
  const calls: unknown[] = [];
  let conflict = "none";
  let current: null | Record<string, unknown> = null;
  ...
  assert.equal(
    ((await pendingRead.json()) as any).polling[0].last_success_at,
    1_000,
  );
  ...
  assert.equal(
    ((await rotated.json()) as { id: string }).id,
    "forgejo-connection",
  );
  ...
  const unhealthyBody = (await unhealthy.json()) as {
    health: string;
