...
} from "./forgejo-integration-support.ts";
...
test("Forgejo verification proves the fixed profile without provider writes", async (context) => {
  const requests: { method: string | undefined; path: string }[] = [];
  let forbiddenRepositoryId: number | null = null;
  let repositoryFailure: "capability" | "git" | null = null;
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal((error as any).code, "forgejo_repository_permission_denied");
      assert.equal((error as any).repositoryId, 11);
      return true;
    },
  );
  forbiddenRepositoryId = 12;
  let partialFailure: any;
  try {
