...
} from "../src/review/review-run-codex-adapter.ts";
import { ReviewRunExecutionError } from "../src/review/review-run-result.ts";
import { runReviewRunCodex } from "./review-run-codex-adapter-support.ts";
...
function processThatExits(code: number, signal: NodeJS.Signals | null = null) {
...
test("constructs the pinned Codex invocation and accepts only the submission channel Result", async () => {
  ...
  await runReviewRunCodex({
    ...
    spawnProcess(command, arguments_, options) {
      ...
      return processThatExits(0) as any;
    },
    ...
    await assert.rejects(
      () =>
        runReviewRunCodex({
          ...
          resultService: { prepare() {} },
          run,
          spawnProcess: () => processThatExits(code) as any,
        }),
      (error) => {
    ...
    await assert.rejects(
      () =>
        runReviewRunCodex({
          ...
          resultService: { prepare() {} },
          run,
          spawnProcess: () => processThatExits(exitCode) as any,
        }),
      (error) => {
        assert.ok(error instanceof ReviewRunExecutionError);
  ...
  await assert.rejects(
    () =>
      runReviewRunCodex({
        ...
        resultService: { prepare() {} },
        run,
        spawnProcess: () => processThatFailsWithJsonl() as any,
      }),
    (error) => {
      assert.ok(error instanceof ReviewRunExecutionError);
      ...
      assert.equal(
        (error.cause as any).stdout,
        '{"type":"turn.failed","error":{"message":"model failure"}}\n',
      );
      assert.equal((error.cause as any).stderr, "pinned Codex diagnostic\n");
      return true;
    },
  );
});
  ...
  await assert.rejects(
    () =>
      runReviewRunCodex({
        ...
        resultService: { prepare() {} },
        run,
        spawnProcess: () => processThatExits(2) as any,
      }),
    (error) => {
      assert.ok(error instanceof ReviewRunExecutionError);
      assert.equal(error.code, "codex_process_failed");
      assert.equal(
        (error as any).submissionChannelCleanupFailure,
        cleanupFailure,
      );
      return true;
    },
...
test("channel cleanup failure remains visible without overturning an accepted Result", async () => {
  ...
  assert.deepEqual(
    await runReviewRunCodex({
      ...
      resultService: { prepare() {} },
      run,
      spawnProcess: () => processThatExits(0) as any,
      killProcessGroup(pid, signal) {
        assert.equal(pid, -76);
        if (signal === 0) {
          throw Object.assign(new Error("process group exited"), {
