...
} from "../src/evaluation/evaluation-collection.ts";
import { readEvaluationMonitors } from "../src/evaluation/evaluation-monitor.ts";
...
function summary(
  ...
  status: string,
  {
    result = null,
    counts = {},
  }: { result?: string | null; counts?: Record<string, number> } = {},
) {
  return {
    evaluation_id: id,
...
function noRuns(id: string, result: string | null = null) {
  return {
    evaluation_id: id,
    finding_advisory: 0,
    finding_blocking: 0,
...
function evaluation(
  ...
  applicabilitySealedAt: number | null = null,
) {
  return {
...
function monitors(
  rows: Record<string, import("node:sqlite").SQLInputValue>[],
  evaluations: Record<string, import("node:sqlite").SQLInputValue>[],
) {
...
test("monitor preserves review query order and exposes immutable result counts", () => {
  ...
  assert.deepEqual(
    value?.nodes
      .slice(1, -1)
      .map((node: { outcome?: string; review_id?: string }) => [
        node.review_id,
        node.outcome,
      ]),
    [
...
test("collection applies every filter together with escaped case-insensitive text matching", () => {
  let statement = "";
  let parameters: import("node:sqlite").SQLInputValue[] = [];
  const reader = createEvaluationCollectionReader(
    {
      all(sql: string, ...values: Array<import("node:sqlite").SQLInputValue>) {
        statement = sql;
        parameters = values;
        return [];
      },
