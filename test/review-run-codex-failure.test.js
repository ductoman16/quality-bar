...
test("maps an uncoded submission-channel failure without exposing its detail", async () => {
  const submissionFailure = new Error(
    "submission socket exposed QUALITY_BAR_SUBMIT_TOKEN=secret",
  );
  await assert.rejects(
    () =>
      runReviewRunCodex({
