...
function addSecondInlineFinding(core) {
  core.run(
...
/** @typedef {{surface: string, source_id: string, attempt_count: number, error_code: string | null, error_detail: string | null, next_attempt_at: number, reconciliation_required: number, external_id: number | null}} DeliveryRow */
/** @typedef {Record<string, any>} FactRow */
/** @typedef {{operations: string[], deliveries: DeliveryRow[], feedback: FactRow[], inline: FactRow[], statuses: FactRow[], deliveryGate: FactRow | null, pollingError: string | null, pollingFetches: number, pollingGate: FactRow | null, pollingRow: FactRow | null}} WorkerOutput */
...
for (const scenario of cases) {
  test(`${scenario.provider} delivery and polling recovery persists across process restart`, (context) => {
    ...
    /** @type {(now: number) => WorkerOutput} */
    const runWorker = (/** @type {number} */ now) =>
      /** @type {WorkerOutput} */ (
        JSON.parse(
          execFileSync(
            process.execPath,
    ...
    const pollingFailure = first.pollingGate ?? {
      ...
      message: /** @type {FactRow} */ (pollingRow).error_message,
      nextAttemptAt: /** @type {FactRow} */ (pollingRow).next_attempt_at,
    };
    assert.equal(
