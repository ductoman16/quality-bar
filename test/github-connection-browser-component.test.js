...
test("GitHub Connection control has semantic live states, deterministic focus, and submits the exact Manifest form", async () => {
  ...
  assert.match(
    page,
    /<section class="qb-region" aria-labelledby="github-connection-title">.*<form id="github-connection-form">.*<textarea hidden id="github-connection-pem"><\/textarea>.*<button[^>]*id="github-connection-submit"[^>]*>Connect GitHub App<\/button>.*<dl>.*<dt>Identity<\/dt>.*<dt>API profile<\/dt>.*<dt>Health<\/dt>.*<dt>Permissions<\/dt>.*<dt>Capabilities<\/dt>.*<dt>Latest verification<\/dt>.*<h4>Verification history<\/h4>.*<form hidden id="github-repository-selection-form">.*<fieldset id="github-repository-selection-fieldset">.*<legend>GitHub Repositories<\/legend>.*<button[^>]*id="github-repository-selection-submit"[^>]*>Register selected Repositories<\/button>.*<output class="qb-status" aria-live="polite" id="github-connection-status" tabindex="-1".*role="alert" tabindex="-1"/,
  );
