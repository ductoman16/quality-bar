...
import { recoverStartedAdjudicationWithFakeCodex } from "./support/waiver-adjudication-recovery-fake-codex.ts";
...
async function runFocusedAdjudication(
...
test("one focused fake Codex process atomically persists a mixed valid Decision set", async (context) => {
  ...
  const transcript = core
    ...
    .map(({ stream, content }: any) => `${stream}:${content}`)
    .join("");
