...
} from "../src/forgejo/forgejo-polling.ts";
...
import { readGitHubPollingGeneration } from "../src/github/github-polling-generation.ts";
...
function openTestCore(context: import("node:test").TestContext, label: string) {
  const directory = mkdtempSync(join(tmpdir(), label));
  context.after(() => rmSync(directory, { force: true, recursive: true }));
  return openDurableCore(join(directory, "quality-bar.sqlite3"));
}

async function establishGitHubRepository(core: any) {
...
async function establishForgejoRepository(core: any) {
...
test("GitHub removal fences in-flight polling success and failure", async (context) => {
  for (const action of ["delete", "retire"]) {
    for (const outcome of ["success", "failure"]) {
      ...
      let failureCommit: ((transaction: any) => boolean) | undefined;
      ...
      try {
        ...
        assert.equal(
          (error as GitHubConnectionError).code,
          "github_repository_api_access_failed",
        );
      }
      await removeRepository(core, "github", action as "delete" | "retire");
...
test("Forgejo removal fences in-flight polling success and failure", async (context) => {
  for (const action of ["delete", "retire"]) {
    for (const outcome of ["success", "failure"]) {
      ...
      await removeRepository(core, "forgejo", action as "delete" | "retire");
      const committed = core.transaction((transaction) =>
        outcome === "success"
