...
import { recoverOperatorAuthorityFromHost } from "../src/operator/operator-authority-recovery.ts";
import {
...
function throwValue(value: unknown) {
  throw value;
}
...
test("the recovery command has no argument or environment password channel", () => {
  const command = readFileSync(
    new URL("../src/recover-operator-authority.ts", import.meta.url),
    "utf8",
  );
