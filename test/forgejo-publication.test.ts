...
import {
  ...
  readEvaluation,
} from "../src/evaluation/evaluation-resource.ts";
import { createForgejoPublicationServices } from "../src/forgejo/forgejo-publication-services.ts";
import { arrangeForgejoFeedback as arrange } from "./forgejo-feedback-publication-support.ts";
...
test("Forgejo completes admitted delivery after Repository disablement", async (context) => {
  ...
  const services = createForgejoPublicationServices(core, {
    ...
    now: () => 10,
    verifier: {
      async publishCommitStatus(...parameters: any[]) {
        statuses.push(parameters);
        return 901;
      },
      async publishAggregateFeedback(...parameters: any[]) {
        aggregates.push(parameters);
        return 902;
      },
      async publishInlineFeedback(...parameters: any[]) {
  ...
  const resourceWithPublication = resource as any;
  ...
  assert.deepEqual(
    resourceWithPublication.feedback.findings.map((finding: any) => ({
...
test("Forgejo retirement makes waiting publication surfaces unavailable without reading credentials", async (context) => {
  ...
  const { retireForgejoConnection } =
    await import("../src/forgejo/forgejo-connection-lifecycle.ts");
