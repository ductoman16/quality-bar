...
} from "../src/github/github-delivery.ts";
import { resumeGitHubDeliveries } from "../src/github/github-delivery-recovery.ts";
...
} from "../src/forgejo/forgejo-delivery.ts";
import { resumeForgejoDeliveries } from "../src/forgejo/forgejo-delivery-recovery.ts";
import { openDurableCore } from "../src/durable/durable-core.ts";
import {
  isDefinitiveGitHubPollingFailure,
  nextGitHubAttemptAt,
} from "../src/github/github-polling-failure.ts";
import {
  isDefinitiveForgejoPollingFailure,
  nextForgejoAttemptAt,
} from "../src/forgejo/forgejo-polling-failure.ts";
import { arrangeForgejoFeedback } from "./forgejo-feedback-publication-support.ts";
import { arrangeGitHubCommitStatus } from "./github-commit-status-publication-support.ts";
...
test("definitive publication failures remain visibly unavailable until correction resumes delivery", async (context) => {
  ...
  for (const scenario of cases) {
    ...
    let now = 0;
    let publishes = 0;
    const service = (scenario.createService as any)(core, {
      cipher: {
        decrypt: () =>
    ...
    core.transaction((transaction: any) => {
