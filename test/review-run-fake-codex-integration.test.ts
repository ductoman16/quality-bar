...
async function proveFakeCodexResult(
  ...
  let signalDeadline: () => void = () =>
    assert.fail("deadline timer was not installed");
  let forceKill: () => void = () =>
    assert.fail("termination timer was not installed");
  ...
  const execution = () =>
    executeReviewRun(core, claim, {
      ...
      ...scenario.createTimerOptions({
        setForceKill(callback: () => void) {
          forceKill = callback;
        },
        setSignalDeadline(callback: () => void) {
