...
import { readSystemStorageFacts } from "../src/system/system-storage-facts.ts";

const NOW = Date.parse("2026-08-02T12:00:00.000Z");
const KEY_IDENTITY = `sha256:${"a".repeat(64)}`;

function backup(createdAt: number) {
  return {
...
function readFacts(backups: any[]) {
  function select({ kind }: { kind?: "daily" } = {}) {
    return backups.filter((backup: any) => backup.kind === kind);
  }
  return select;
}
