...
import { ensureForgejoDelivery } from "../src/forgejo/forgejo-delivery.ts";
...
test("SQLite retirement blocks enabled and disabled dependents, then destroys only the PAT", async (context) => {
  ...
  assert.equal(
    (service.retire({ lifecycle: "retired" }) as { lifecycle: string })
      .lifecycle,
    "retired",
  );
  assert.equal(
...
test("SQLite reactivation completely verifies the same Forgejo identity and restores a write-only PAT", async (context) => {
  ...
  assert.equal((service.read() as { lifecycle: string }).lifecycle, "retired");
  assert.equal(
    core.get("SELECT count(*) AS count FROM forgejo_connection_credentials")
      ?.count,
    0,
  ...
  assert.equal((reactivated as { lifecycle: string }).lifecycle, "enabled");
  assert.equal((reactivated as any).health_error, null);
  assert.deepEqual(
    core.get(
  ...
  assert.deepEqual((reactivated as any).capabilities, {
  ...
  assert.equal((reactivated as any).verification_history.length, 4);
  assert.deepEqual(
    core.get(
