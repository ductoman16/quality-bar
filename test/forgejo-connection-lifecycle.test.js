...
test("Forgejo retirement decisions and PAT destruction are unit-owned", () => {
  ...
  assert.match(writes[4], /DELETE FROM forgejo_delivery_provider_gates/);
  assert.match(writes[5], /DELETE FROM forgejo_connection_credentials/);
  const neverUsed = {
    ...core,
    all(/** @type {string} */ sql) {
...
test("Forgejo reactivation requires complete proof and records failure", async () => {
  ...
  assert.ok(
    successWrites.some((sql) => sql.includes("forgejo_commit_statuses")),
  );
