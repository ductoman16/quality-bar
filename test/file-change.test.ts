...
import { EvaluationError } from "../src/evaluation/evaluation-validation.ts";
import { fileChangesFromGitNameStatus } from "../src/file-change.ts";
import { createGitPathMatcher } from "../src/git-path-matcher.ts";

function evaluate(fileChange: any) {
...
test("one authored Git pathspec expansion serves every touched path", () => {
  const calls: { commit: string; pathspec: string }[] = [];
