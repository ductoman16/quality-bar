...
import { addWaiverFollowupFacts } from "./system-polling-delivery-support.ts";
...
test("the authenticated System API preserves waiver ownership before delivery starts", async () => {
  ...
  const byOwner = new Map<string, any>(
    system.delivery.surfaces
      .filter((surface: any) => surface.source_identity.startsWith("waiver-"))
      .map((surface: any) => [surface.owner_kind, surface]),
  );
  assert.deepEqual(
    {
