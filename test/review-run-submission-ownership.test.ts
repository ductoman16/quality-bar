...
import { processPendingResponse } from "../src/review/review-run-submission-pending.ts";
import { ReviewRunExecutionError } from "../src/review/review-run-result.ts";
import { requirePrivateFile } from "../src/review/review-run-submission-files.ts";
import { settleSubmissionTerminal } from "../src/review/review-run-codex-submission-terminal.ts";
...
test("shares the acknowledgment inode before stopping acceptance", () => {
  ...
  let sharedIdentity: { dev: number; ino: number } | null = null;
  let stoppedIdentity: { dev: number; ino: number } | null = null;
...
test("a response publication failure cannot overturn a committed Result", async () => {
  ...
  let preparedCandidate: unknown = null;
  const processSubmission = createSubmissionProcessor({
    ...
    requestPath,
    resultService: {
      prepare: (_claim: any, submittedCandidate: unknown) => {
        void _claim;
