...
test("schema v29 gains the whole-lineage deletion guard without weakening Review Version immutability", (context) => {
  const directory = mkdtempSync(join(tmpdir(), "quality-bar-review-v29-"));
  context.after(() => rmSync(directory, { force: true, recursive: true }));
  ...
  const reviews = createReviewService(current, {
    createId: (() => {
      let nextId = 0;
      return () => `review-v29-fact-${++nextId}`;
    })(),
    now: () => 1,
  });
  const unused = reviews.create(definition("Migrated never-used Review"));
  current.transaction((transaction) => {
    for (const trigger of [
      "review_hard_delete_lineage",
      "review_hard_delete_marker_insert",
      "review_versions_immutable_delete",
      "criteria_immutable_delete",
      "review_version_criteria_immutable_delete",
    ]) {
      transaction.run(`DROP TRIGGER ${trigger}`);
    }
    transaction.run("ALTER TABLE reviews DROP COLUMN hard_delete_pending");
    transaction.run(`
    ...
    `);
    transaction.run(
      "UPDATE quality_bar_metadata SET value = '29' WHERE key = 'schema_version'",
    );
    transaction.run("PRAGMA user_version = 29");
  });
  current.close();
  ...
  assert.deepEqual(
    ...
    [
      { name: "review_hard_delete_lineage" },
      { name: "review_hard_delete_marker_insert" },
    ],
  );
  assert.throws(
    () =>
  ...
  createReviewService(migrated).remove(unused.id, {});
  assert.equal(migrated.get("SELECT count(*) AS count FROM reviews")?.count, 0);
});
