...
import { createHardStorageBoundary } from "../src/application/application-runtime.ts";
...
import { prepareCodexProcess } from "../src/codex/codex-process-supervisor.ts";
...
function readLiveProcessGroupMembers(processGroupId: number): number[] {
...
async function assertProcessGroupHasNoLiveMembers(
...
test("hard storage failure force-kills a resistant supervised process group", async (context) => {
  ...
  await assertProcessGroupHasNoLiveMembers(child.pid as number);
});
...
test("restart rejects a tracked process group whose identity anchor exited", async (context) => {
  ...
  const descendantReady: Promise<void> = new Promise((resolve, reject) => {
    leader.once("error", reject);
    leader.once("message", (message) => {
      descendantId = (message as { childPid: number }).childPid;
      resolve();
    });
  });
  context.after(() => {
    for (const target of [-(leader.pid as number), descendantId]) {
      try {
        if (Number.isSafeInteger(target)) {
          process.kill(target as number, "SIGKILL");
        }
      } catch (error) {
        if (
          !(
  ...
  await leaderExited;
  assert.doesNotThrow(() => process.kill(-(leader.pid as number), 0));
