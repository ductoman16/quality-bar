...
import { readEvaluationWaiverAdjudications } from "../src/waiver/waiver-adjudication-resource.js";
...
test("SQLite admits and GitHub publishes aggregate plus accepted original-inline follow-ups", async () => {
  ...
  try {
    ...
    const statusService = createGitHubCommitStatusService(core, {
      ...
      ioPool: {
        run: (/** @type {any} */ duty, /** @type {any} */ operation) => {
          void duty;
          return operation();
        },
      },
    ...
    const service = createGitHubWaiverFollowupService(core, {
      ...
      verifier: {
        ...
        /** @param {...any} parameters */
        async publishReviewCommentReply(...parameters) {
