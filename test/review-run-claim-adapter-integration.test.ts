...
import { executeClaimWithOwningAdapter } from "../src/codex/codex-execution-dispatch.ts";
import { createCodexExecutionClaimService } from "../src/codex/codex-execution-claim.ts";
import { startSpawnedCodexProcessGroup } from "../src/codex/codex-execution-process-group-tracking.ts";
import { createIoExecutionPool } from "../src/io-execution-pool.ts";
import { runReviewRunCodex as runProductionReviewRunCodex } from "../src/review/review-run-codex-adapter.ts";
import { seedQueuedCodexExecutionKinds } from "./codex-execution-ordering-support.ts";
import {
  acceptedChannel,
...
test("tracking failure still terminates the untracked detached process group", async () => {
  ...
  assert.equal((trackingFailure as any).submissionCloseFailure, closeFailure);
  assert.equal((trackingFailure as any).terminationFailure, terminationFailure);
});
...
test("a termination failure retains tracked provenance for recovery", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal((error as any).terminationFailure, terminationFailure);
      return true;
    },
  );
...
test("a falsy termination failure retains tracked provenance for recovery", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.equal(error, bindingFailure);
      assert.match(
        (error as any).terminationFailure.message,
...
test("the owning fake Codex adapter is reached only after the shared durable claim commits", (context) => {
  ...
  function launchFakeCodex(claim: { workId: string }) {
...
test("a long owning Codex adapter cannot starve an independent I/O duty", async () => {
  let finishCodex: (value?: void) => void = () => {};
