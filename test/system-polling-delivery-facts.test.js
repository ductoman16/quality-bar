...
/** @param {any} core @param {string} repositoryId @param {number} forgeRepositoryId @param {string} name */
function addPollingRepository(core, repositoryId, forgeRepositoryId, name) {
  core.run(
...
/** @param {any} core @param {number} forgeRepositoryId @param {string} baselineStatus @param {number | null} lastSuccessAt @param {string | null} errorCode @param {string | null} errorMessage @param {number | null} rateGateUntil @param {number | null} nextAttemptAt @param {string | null} snapshot */
...
test("System exposes durable polling baselines and separate delivery outcomes with owning resources", (context) => {
  ...
  assert.deepEqual(
    connection.repositories.map((/** @type {any} */ repository) => ({
