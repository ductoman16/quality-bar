...
test("an onboarding token can mutate only its bound Repository and revoke itself", async () => {
  ...
  const token = /** @type {{token: string}} */ (await tokenResponse.json())
    .token;
  const headers = {
  ...
  /** @param {string} name @param {string} repositoryId */
  const createReview = async (name, repositoryId) => {
  ...
  assert.deepEqual(
    /** @type {{assignment: unknown}} */ (await created.json()).assignment,
