...
import { createCodexExecutionWorker } from "../src/codex/codex-execution-worker.ts";
import { createStorageGuardedClaimService } from "../src/codex/codex-execution-runtime.ts";

test("the application worker fills durable claims without serializing long executions", async () => {
  const claims: import("../src/codex/codex-execution-claim.ts").CodexExecutionClaim[] =
    [
      {
        fencingToken: 1,
