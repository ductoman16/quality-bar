...
test("registered Fastify schemas own validation and OpenAPI 3.1", async () => {
  ...
  assert.equal(
    ((await authenticatedUnknown.json()) as any).error.code,
    "not_found",
  );
  ...
  assert.equal(
    ((await machineLogin.json()) as any).error.code,
  ...
  assert.equal(((await badOrigin.json()) as any).error.code, "origin_invalid");
  ...
  assert.equal(
    ((await missingIdempotencyKey.json()) as any).error.code,
  ...
  const error = (await tooLarge.json()) as any;
