...
} from "../src/application/application-shutdown.ts";
...
import { evaluationFailureStatus } from "../src/evaluation/evaluation-route-failure.ts";
import { isUnavailableError } from "../src/http-request.ts";
...
test("graceful close aborts bounded I/O and retains credentials until accepted work drains", async () => {
  const order: string[] = [];
  const codexDrain = Promise.withResolvers() as PromiseWithResolvers<void>;
  const ioDrain = Promise.withResolvers() as PromiseWithResolvers<void>;
  ...
  const close = createApplicationClose({
    ...
    server: { server: { listening: false } } as any,
    shutdownBoundary,
...
test("graceful close never reports completion when a finalizer fails", async () => {
  ...
  const hostLogs: Record<string, any>[] = [];
  const close = createApplicationClose({
    codexRuntime: null,
    durableCore: { close() {} },
    evaluations: null,
    ...
    writeLog: Object.assign(() => {}, {
      host: (line: string) => hostLogs.push(JSON.parse(line)),
    }),
  });
