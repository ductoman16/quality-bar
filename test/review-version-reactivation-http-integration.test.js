...
test("the authenticated active Review Version resource reactivates history without creating a version", async () => {
  ...
  const created =
    /** @type {{id: string, active_version: {id: string, codex_configuration: object, criteria: Array<{id: string, impact: string, instruction: string}>}}} */ (
  ...
  const reactivated =
    /** @type {{changed: boolean, review: {active_version: {id: string}, versions: Array<{id: string}>}}} */ (
      await reactivatedResponse.json()
    );
