...
import { stubUnavailableReviewService } from "./service-stubs-support.ts";
import {
  responseErrorCode,
  reviewRequest,
  sessionCookies,
...
test("the authenticated Review resource creates only an exact complete v1 snapshot", async () => {
  ...
  const createdReview = (await created.json()) as {
    active_version: { number: number };
  };
...
test("the authenticated Review metadata resource edits only lineage metadata", async () => {
  ...
  const created = (await createdResponse.json()) as {
    id: string;
    active_version: { id: string; number: number };
  };
...
test("the authenticated Review Version resource saves a complete snapshot or explicitly does nothing", async () => {
  ...
  const savedResult = (await savedResponse.json()) as {
    changed: boolean;
    review: {
      active_version: {
        id: string;
...
test("an unexpected Review resource failure has an exact owning error", async () => {
  ...
  const listedFailure = (await listed.json()) as {
  ...
  const unavailableFailure = (await unavailableList.json()) as {
    error: { code: string; message: string };
  };
  ...
  const invalidFailure = (await invalidList.json()) as {
    error: { code: string; message: string };
  };
  ...
  const versionFailure = (await versionResponse.json()) as {
    error: { code: string; message: string };
  };
