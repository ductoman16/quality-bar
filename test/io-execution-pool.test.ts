...
import { createGitHubApiRequest } from "../src/github/github-api-request.ts";
import {
...
test("the fixed I/O pool bounds independent duties without hiding failures", async () => {
  ...
  function createTask(duty: (typeof duties)[number]) {
...
test("hard shutdown aborts production provider I/O before the pool drains", async () => {
  ...
  const request = createGitHubApiRequest("https://api.github.test", ((
    _url: unknown,
