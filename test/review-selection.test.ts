...
import { selectReviewVersionsForNewEvaluation } from "../src/review/review-selection.ts";
...
test("new Evaluation selection rejects partial Review state", () => {
  for (const reviews of [
    ...
    assert.throws(
      () =>
        selectReviewVersionsForNewEvaluation(
          reviews as Parameters<typeof selectReviewVersionsForNewEvaluation>[0],
          "repository-1",
        ),
...
test("new Evaluation selection rejects a missing Repository identity instead of skipping scoped Reviews", () => {
  assert.throws(
    () => selectReviewVersionsForNewEvaluation([], undefined as any),
