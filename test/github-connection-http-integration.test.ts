...
import { stubUnavailableGitHubConnectionService } from "./service-stubs-support.ts";
  ...
  return {
    ...
    create() {
      return {
        ...
        async selectRepositories(input: any) {
...
test("GitHub callbacks return the exact owning error to the operator surface without inferred success", async () => {
  let callbackFailure: { code: string; message: string } | null = null;
  const { request } = await startApplication({
    createGitHubConnections: () => ({
      startPolling() {},
      read() {
      ...
      recordCallbackFailure(error: GitHubConnectionError) {
