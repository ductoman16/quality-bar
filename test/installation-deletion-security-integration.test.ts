...
import { deleteStoppedInstallation } from "../src/installation-deletion.ts";

function mappedFilesystem(root: string) {
  const paths = new Map([
  ...
  return {
    ...
    readdir: (path: string) => readdirSync(map(path), { withFileTypes: true }),
    remove: (path: string) =>
      rmSync(map(path), { force: true, recursive: true }),
  };
}
