...
function machineHeaders(
  application: { implementerTokens: { create(password: string): string } },
...
test("the implementer token requests, polls, and reads canonical Evaluation facts", async () => {
  ...
  assert.equal(
    ((await queued.json()) as { execution_status: string }).execution_status,
    "queued",
  );
  ...
  const waiverResource = (await waiver.json()) as any;
...
test("disconnect after durable machine acceptance does not cancel Evaluation work", async () => {
  let reportAccepted: (value?: unknown) => void = () => {};
  ...
  const disconnected: Promise<void> = new Promise((resolve, reject) => {
