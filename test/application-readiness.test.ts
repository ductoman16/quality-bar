...
import { loadInstallationConfiguration } from "../src/installation-configuration.ts";
import { acquireInstallationLock } from "../src/installation-environment.ts";
...
function availableStorageReserve(): ReturnType<
...
async function startApplication(
  databasePath: string,
  options: Partial<Parameters<typeof createApplication>[0]> = {},
) {
...
test("unavailable Codex authentication leaves the durable System surface ready", async () => {
  ...
  const system = (await systemResponse.json()) as any;
