 ...
 */
async function submitWithoutClientInspection(channel, checkoutPath) {
 ...
 * @param {string} checkoutPath
 */
async function submitDetached(channel, checkoutPath) {
  writeFileSync(join(checkoutPath, ".quality-bar-result.json"), "{}\n");
  return await new Promise((resolve) => {
    const child = spawn(
      join(channel.commandDirectory, "quality-bar-submit"),
...
test(
  ...
  async (context) => {
    const checkoutPath = mkdtempSync(join(tmpdir(), "qbs-provenance-"));
    context.after(() => rmSync(checkoutPath, { force: true, recursive: true }));
    const trustedGroup = /** @type {number} */ (
      processGroupIdentity(process.pid)
    );
    /** @type {Array<{group: number | null, pid: number}>} */
    const inspections = [];
    const channel = await openReviewRunSubmissionChannel(
      claim,
      { prepare() {} },
