...
test("issue 112 proof is owned by unit, browser-component, and HTTP-integration gates without another cross-process smoke", () => {
  const evidence = JSON.parse(
    readFileSync(
      resolve(
        import.meta.dirname,
        "../evidence/quality-foundation/issue-112-system-storage.json",
      ),
      "utf8",
    ),
  );
  ...
  assert.equal(evidence.ticket, 112);
  assert.deepEqual(evidence.acceptance_scenarios, ["ACC-01", "ACC-10"]);
