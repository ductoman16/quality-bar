...
const EVENT_COUNT_NAMES = Object.freeze(
  /** @type {Record<string, "itemCompleted" | "itemStarted" | "threadStarted" | "turnCompleted" | "turnFailed" | "turnStarted">} */ ({
...
/** @param {unknown} error */
export function safeProcessDiagnostics(error) {
  const process =
    /** @type {{code?: unknown, signal?: unknown, stderr?: unknown, stdout?: unknown} | null} */ (
      error &&
      typeof error === "object" &&
      "cause" in error &&
      error.cause &&
...
/** @param {string} stdout @param {number | null} acceptedAt */
export function readPaidCodexEventEvidence(stdout, acceptedAt) {
  let submissionCommandCompleted = false;
  let submissionCommandFailed = false;
  /** @type {{command: string, id: string} | null} */
  let submissionCommandStarted = null;
