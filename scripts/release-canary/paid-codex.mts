...
import { buildCodexHostEnvironment } from "../../src/review/review-run-codex-command.ts";
import { removeOwnedDirectory } from "../../src/review/review-run-submission-file-cleanup.ts";
import { runReviewRunCodex } from "../../src/review/review-run-codex-adapter.ts";
import {
...
function owningFailureCode(error: unknown) {
  return error &&
    typeof error === "object" &&
    "code" in error &&
...
function safeFailureDetail(error: unknown) {
  ...
  return (
    SAFE_OWNING_DETAILS[
      owningFailureCode(error) as keyof typeof SAFE_OWNING_DETAILS
...
function validateCandidate(candidate: unknown) {
...
export async function invokePaidCodexCanary({
  ...
  sourceCommit: string;
  sourceStatus: string;
  trackProcessGroup?: (processGroupId: number) => void;
}) {
  const startedAt = now();
  const common = {
    kind: KIND,
  ...
  try {
    ...
    let accepted = false;
    let acceptedAt = null as number | null;
