...
import { proveBundledTools } from "./bundled-tools.mts";
import { provePackageOfflineRestore } from "./offline-restore.mts";
import { proveOwnedArtifactCleanup } from "./owned-artifact-cleanup.mts";
import { proveOperatorAuthorityRecovery } from "./operator-authority-recovery.mts";
import { proveRetention } from "./retention.mts";
import { proveInstallationDeletion } from "./installation-deletion.mts";
import { assertFilesystemFacts } from "./filesystem-facts.mts";
import { proveGracefulShutdown } from "./graceful-shutdown.mts";
...
export type ComposeConfiguration =
  import("./compose-configuration.mts").ComposeConfiguration;

export type FilesystemFacts = {
  localFilesystems: boolean;
  stateFreeBytes: number;
  checkoutsFreeBytes: number;
  pathFacts: Record<
    string,
    { filesystemType: number; gid: number; mode: number; uid: number }
  >;
};
...
export type DatabaseFacts = ReturnType<
  typeof provePackageOfflineRestore
>["restoredDatabaseFacts"];

export type AuthenticatedHttpSmoke = ReturnType<
  typeof provePackageOfflineRestore
>["authenticatedHttpSmoke"];

export type DurableWriteFailureFacts = {
...
export type InstallationDeletionFacts = ReturnType<
  typeof proveInstallationDeletion
>;

export type GracefulShutdownFacts = ReturnType<typeof proveGracefulShutdown>;
...
export function proveComposeService({
  configuration,
  fixture,
}: {
  configuration: ComposeConfiguration;
  ...
  assert.equal(processArguments.at(-1), "src/main.ts");

  const filesystemFacts = jsonPackageProbe(
    fixture,
    "filesystem-facts.mjs",
  ) as FilesystemFacts;
  assertFilesystemFacts(filesystemFacts);
  const httpFacts = jsonPackageProbe(fixture, "http-facts.mjs", [
  ...
  const initialDatabaseFacts = jsonPackageProbe(
    fixture,
    "database-facts.mjs",
  ) as DatabaseFacts;
  ...
  const recreatedDatabaseFacts = jsonPackageProbe(
    fixture,
    "database-facts.mjs",
  ) as DatabaseFacts;
  const backupFacts = jsonPackageProbe(fixture, "backup-facts.mjs", [
    fixture.masterKey,
  ]) as BackupFacts;
  ...
  const initialAuthenticatedHttpSmoke = jsonPackageProbe(
    fixture,
  ...
  const durableWriteFailure = jsonPackageProbe(
    fixture,
    "durable-write-failure.mjs",
    [environment.QUALITY_BAR_HTTP_PORT],
    restorePassword,
  ) as DurableWriteFailureFacts;
  ...
  fixture.runCompose(["up", "--detach", "--wait", "--force-recreate"]);
  const repairedHttpFacts = jsonPackageProbe(fixture, "http-facts.mjs", [
    environment.QUALITY_BAR_HTTP_PORT,
  ]) as HttpFacts;
  ...
  const facts = packageFacts({
    ...
    installationDeletion,
    processArguments,
    recoveryDatabaseFacts: recoveryDatabaseFacts as DatabaseFacts,
    recoveryPasswordStatus,
    restoredDatabaseFacts,
    restorePasswordStatus,
    toolVersions,
